...
def nanobot_mcp_tool_name(server_name: object, raw_tool_name: object) -> str:
    """Mirror nanobot's MCP tool-name normalization for exact allowlisting."""
...
def build_runtime_capability_policy(
    ...
    *,
    ...
    """Create the wire policy and opaque grant consumed by nanobot."""
    runtime = manifest.get("runtimePolicy")
    runtime = runtime if isinstance(runtime, Mapping) else {}
    gateway_tool_ids = _normalized_ids(runtime.get("gatewayToolIds"))
    nanobot_tool_ids = _normalized_ids(runtime.get("nanobotToolIds"))
    ...
    policy_material = {
        ...
        "nanobotToolIds": nanobot_tool_ids,
        "skillIds": skill_ids,
    ...
    return {
        ...
        "policy_id": policy_id,
        "allowed_tools": nanobot_tool_ids,
