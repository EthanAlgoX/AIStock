...
def build_agent_chat_executor(
    ...
    *,
    ...
    if backend_id in {"codex_app_server", "external_runtime"} and arch != "single":
    ...
    if backend_id == "litellm":
        ...
        from src.agent.external_agent_backend import ExternalAgentBackend

        context_llm_adapter = None
        backend = ExternalAgentBackend(config)
