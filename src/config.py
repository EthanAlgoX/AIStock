...
SUPPORTED_AGENT_BACKENDS = {"auto", "litellm", "codex_app_server", "external_runtime"}
...
class Config:
    ...
    # 大盘复盘市场区域：cn(A股)、hk(港股)、us(美股)、jp(日股)、kr(韩股)、tw/gb/ca/au/in/de/fr；both(原五市场)
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            agent_runtime_api_base=(os.getenv('AGENT_RUNTIME_API_BASE') or '').strip().rstrip('/'),
            agent_runtime_api_key=(os.getenv('AGENT_RUNTIME_API_KEY') or '').strip(),
    ...
    def _parse_report_language(cls, value: Optional[str]) -> str:
        ...
        if raw and not is_supported_report_language_value(raw):
            logging.getLogger(__name__).warning(
                "REPORT_LANGUAGE '%s' invalid, fallback to 'zh' (valid: zh/en/ko/ja/zh-TW)",
                value,
            )
        return normalized
    ...
    def _parse_market_review_region(cls, value: str) -> str:
        ...
        logging.getLogger(__name__).warning(
            f"MARKET_REVIEW_REGION 配置值 '{value}' 无效，已回退为默认值 'cn'（合法值：cn / hk / us / jp / kr / tw / gb / ca / au / in / de / fr / both；支持逗号分隔有效值）"
        )
        return 'cn'
    ...
    def validate_structured(self) -> List[ConfigIssue]:
        ...
        if agent_backend not in SUPPORTED_AGENT_BACKENDS:
            issues.append(ConfigIssue(
                severity="error",
                message=(
                    "AGENT_BACKEND 当前支持 auto、litellm、codex_app_server、external_runtime。"
                    f"已配置的值为：{agent_backend}。"
                ),
                field="AGENT_BACKEND",
                code="capability_unsupported",
            ))
        if agent_backend == "external_runtime" and not self.agent_runtime_api_base:
            issues.append(ConfigIssue(
                severity="error",
                message="使用独立 Agent 引擎时必须配置 AGENT_RUNTIME_API_BASE。",
                field="AGENT_RUNTIME_API_BASE",
                code="invalid_config",
            ))
        if agent_backend in {"codex_app_server", "external_runtime"} and self.agent_arch != "single":
            issues.append(ConfigIssue(
                severity="error",
                message=(
