        ...
        try:
            ...
            if task.strategy_version_id:
                ...
                if kernel_result.get("status") != "success":
                    raise RuntimeError(str(kernel_result.get("message") or "策略内核未生成研究报告"))
                result = kernel_result.get("result")
                if isinstance(result, dict):
                    result["strategy_version_id"] = task.strategy_version_id
            ...
            if result:
                ...
                if task and task.strategy_version_id:
                    try:
                        from src.storage import DatabaseManager

                        DatabaseManager.get_instance().attribute_analysis_history_to_strategy(
                        ...
                        logger.warning(
                            "[TaskQueue] 策略研究历史归属写入失败: task_id=%s error=%s",
                            task_id,
                            attribution_error,
                        )
