...
_STRATEGY_DATA_SOURCE_CONFIG: ContextVar[Optional[Dict[str, Any]]] = ContextVar(
    "strategy_data_source_config",
    default=None,
)
...
class ScreeningService:
    ...
    def screen(
        ...
        *,
        ...
        if data_source_config is None:
            ...
            selected, dsa_enrichment = _enrich_candidates_with_dsa(selected, data_source_config=data_source_config)
    ...
    def enrich_candidates(
        ...
        *,
        ...
        """Prepare news/fundamental evidence for an already-fixed strategy universe."""
        return _enrich_candidates_with_dsa(candidates, data_source_config=data_source_config)
...
def _call_screening_screen(
    ...
    *,
    ...
    with _screening_runtime_env(config, max_results=max_results, data_source_config=data_source_config):
        pipeline_config = ScreeningPipelineConfig.from_env()
        pipeline_context = _build_screening_context(config, max_results=max_results)

    daily_history_fetcher = _build_screening_dsa_daily_history_fetcher(data_source_config=data_source_config)
...
@contextmanager
def _screening_runtime_env(config: Config, *, max_results: Optional[int] = None, data_source_config: Optional[Dict[str, Any]] = None) -> Iterator[None]:
    updates = _build_screening_runtime_env(config, max_results=max_results, data_source_config=data_source_config)
    if not updates:
        yield
        return
...
def _build_screening_dsa_daily_history_fetcher(*, data_source_config: Optional[Dict[str, Any]] = None) -> Optional[Callable[..., Any]]:
    ...
    kline_config = (data_source_config or {}).get("kline")
    if not isinstance(kline_config, dict):
        kline_config = {}
    kline_connection = str(kline_config.get("connection") or "system_market_data")
    ...
    def fetch_daily_history_with_dsa(
        ...
        *,
        ...
        try:
            if kline_connection == "system_market_data":
                dsa_df, dsa_source = get_dsa_daily_history(code, lookback_days=lookback_days)
            else:
                dsa_df, dsa_source = get_dsa_daily_history(code, lookback_days=lookback_days, connection=kline_connection)
        ...
        if kline_connection == "local_stock_daily" or kline_connection.startswith("kline:"):
            import pandas as pd
...
def _build_screening_runtime_env(config: Config, *, max_results: Optional[int] = None, data_source_config: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
    ...
    kline_connection = str(kline_config.get("connection") or "system_market_data")
    daily_source = {
        "kline:tencent": "tencent",
        "kline:akshare": "akshare",
        "kline:baostock": "baostock",
        "kline:yfinance": "yfinance",
        "kline:tushare": "tushare",
    }.get(kline_connection, "auto")
    put("DAILY_SOURCE", daily_source)
...
def get_dsa_daily_history(stock_code: str, *, lookback_days: int = 120, connection: str = "system_market_data") -> Tuple[Any, str]:
    from src.services.history_loader import load_history_df

    normalized_code = _env_text(stock_code).zfill(6)
    days = max(int(lookback_days or 0), 30)
    return load_history_df(normalized_code, days=days, connection=connection)
...
def get_dsa_fundamental_context(stock_code: str) -> Dict[str, Any]:
    manager = _get_dsa_fetcher_manager()
    data_sources = _STRATEGY_DATA_SOURCE_CONFIG.get() or {}
    fundamental_config = data_sources.get("fundamentals")
    connection = str(fundamental_config.get("connection") or "system_fundamentals") if isinstance(fundamental_config, dict) else "system_fundamentals"
    if connection == "fundamentals:yfinance":
        from data_provider.base import _market_tag

        market = _market_tag(stock_code)
        context = manager._build_offshore_fundamental_context(stock_code, market=market, budget_seconds=4.0)
    elif connection == "fundamentals:akshare" and not str(stock_code).strip().isdigit():
        context = manager.build_failed_fundamental_context(stock_code, "AkShare 基本面仅支持 A 股代码。")
    else:
...
def search_dsa_stock_news(stock_code: str, stock_name: str = "", max_results: int = 3) -> Dict[str, Any]:
    ...
    response = service.search_stock_news(stock_code, stock_name or stock_code, max_results=max_results, provider_name=provider_name)
...
def search_dsa_stock_events(stock_code: str, stock_name: str = "", max_results: int = 3) -> Dict[str, Any]:
    ...
    response = service.search_stock_events(stock_code, stock_name or stock_code, provider_name=provider_name)
    return _normalize_dsa_search_response(response, max_results=max_results)
...
def _enrich_candidates_with_dsa(
    ...
    *,
    ...
    news_config = (data_source_config or {}).get("news")
    fundamentals_config = (data_source_config or {}).get("fundamentals")
    include_news = not isinstance(news_config, dict) or news_config.get("enabled") is not False
    include_fundamentals = not isinstance(fundamentals_config, dict) or fundamentals_config.get("enabled") is not False
    token = _STRATEGY_DATA_SOURCE_CONFIG.set(data_source_config or {})
    ...
    try:
        for index, candidate in enumerate(candidates):
            ...
            if (
                ...
                and (not include_news or _candidate_has_dsa_news(candidate))
            ):
                enriched_count += 1
        ...
        _STRATEGY_DATA_SOURCE_CONFIG.reset(token)
