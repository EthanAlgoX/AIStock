...
def add_error_handlers(app) -> None:
    ...
    async def http_exception_handler(request: Request, exc: HTTPException):
        ...
        # Preserve structured domain details.  Strategy-definition clients use
        # VERSION_CONFLICT and its server revision to stop autosave safely.
        if isinstance(exc.detail, dict) and "code" in exc.detail and "message" in exc.detail:
            return JSONResponse(
                status_code=exc.status_code,
                content={"error": "http_error", "message": exc.detail["message"], "detail": exc.detail},
            )
