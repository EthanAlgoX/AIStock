...
def runtime_manifest(request: Request) -> dict[str, Any]:
    ...
    return {
        ...
        "taskCapabilityIsolation": {
            "version": 1,
            "mode": "required_for_external_runtime",
        ...
        "notes": [
            "The independent Agent engine owns its ReAct loop, sessions, memory, recovery and runtime-local MCP processes.",
...
async def financial_mcp(request: Request, response: Response) -> Any:
    ...
    if method == "tools/call":
        ...
        execution_context = ToolAccessContext(
            ...
            # bounded by the independent runtime's MCP client timeout. Giving them an internal
