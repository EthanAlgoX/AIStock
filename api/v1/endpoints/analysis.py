...
def trigger_analysis(
    ...
    """
    # Product pages must bind the real complete strategy version before any
    # expensive task is accepted. Legacy callers without a version remain
    # compatible with the original analysis tool.
    strategy_version_id = getattr(request, "strategy_version_id", None)
    if strategy_version_id:
        try:
            version = StrategyDefinitionService().get_version(strategy_version_id)
        ...
        if (
            ...
            or version.get("strategyPurpose") != "research_report"
            or version.get("outputContract") != "ResearchReport"
            or not isinstance(version.get("strategyPackage"), dict)
            or version["strategyPackage"].get("executionStatus") != "ready"
        ):
            raise api_error(409, "research_strategy_version_invalid", "请选择已绑定可调用 Python 内核的正式完整单股研究策略版本")
...
def _handle_async_analysis_batch(
    ...
    if strategy_version_id is not None:
        submit_kwargs["strategy_version_id"] = strategy_version_id
    ...
    try:
        ...
        if strategy_version_id:
            kernel_result = StrategyKernelExecutorService().execute_product(
                strategy_version_id,
                {"symbol": stock_code, "stockCode": stock_code},
                mode="research",
            )
            result = kernel_result.get("result") if kernel_result.get("status") == "success" else None
            if isinstance(result, dict):
                result["strategy_version_id"] = strategy_version_id
        else:
...
def get_task_list(
    ...
    strategy_version_id: Optional[int] = Query(None, ge=1, description="只读取指定正式策略版本的运行记录"),
) -> TaskListResponse:
    """
    获取分析任务列表
    ...
    # 获取所有任务
    all_tasks = task_queue.list_all_tasks(limit=100 if version_filter else limit)
    if version_filter is not None:
        all_tasks = [t for t in all_tasks if getattr(t, "strategy_version_id", None) == version_filter]
    ...
    task_infos = [
        TaskInfo(
            ...
            region=t.region,
            strategy_version_id=getattr(t, "strategy_version_id", None),
        )
        for t in all_tasks
    ]
    ...
    if version_filter is not None:
        ...
        known_ids = {item.task_id for item in task_infos}
        for row in DatabaseManager.get_instance().list_strategy_analysis_runs(version_filter, limit=limit):
            query_id = str(row.get("query_id") or row.get("id") or "")
            if not query_id or query_id in known_ids:
                continue
            task_infos.append(TaskInfo(
                ...
                progress=100,
                message="研究报告已保存",
                report_type=str(row.get("report_type") or "detailed"),
        ...
        task_infos.sort(key=lambda item: item.created_at or "", reverse=True)
        task_infos = task_infos[:limit]
        stats = {
            "total": len(task_infos),
            "pending": sum(item.status == TaskStatusEnum.PENDING for item in task_infos),
            "processing": sum(item.status == TaskStatusEnum.PROCESSING for item in task_infos),
        }
