...
describe('screeningApi', () => {
  ...
  it('sends the selected formal StrategyVersion with the screening request', async () => {
    post.mockResolvedValueOnce({
      data: {
        task_id: 'screen-task-versioned', trace_id: 'screen-task-versioned', status: 'pending',
        message: '选股任务已提交', strategy: 'dual_low', market: 'cn', max_results: 3,
        strategy_version_id: 120,
      },
    });

    await screeningApi.startScreen({ market: 'cn', strategy: 'dual_low', maxResults: 3, strategyVersionId: 120 });
