...
export type WorkspaceTask = { id:string; kind:WorkspaceTaskKind; name:string; market:"CN"|"HK"|"US"|"TW"|"JP"|"KR"|"GB"|"CA"|"AU"|"IN"|"DE"|"FR"|"GLOBAL"; objective:string; subject:Record<string,unknown>; config:Record<string,unknown>; capabilities:AgentCapabilityBindings; version:number; enabled:boolean; createdAt:string; updatedAt:string };
...
export const workspaceApi = {
  ...
  async runHistory(params: { kind?:string; status?:string; query?:string; stock?:string; market?:string; start?:string; end?:string; offset?:number; limit?:number } = {}) { const { data } = await apiClient.get<{ items:WorkspaceRun[]; total:number; statusCounts:Record<string,number> }>(`${root}/run-history`, {params}); return data; },
