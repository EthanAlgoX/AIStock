...
export const analysisApi = {
  ...
  analyze: async (data: AnalysisRequest): Promise<AnalyzeResponse> => {
    const requestData = {
      ...
      ...(data.strategyVersionId !== undefined && { strategy_version_id: data.strategyVersionId }),
  ...
  getTasks: async (params?: {
    ...
    const response = await apiClient.get<Record<string, unknown>>(
      ...
      { params: params ? { status: params.status, limit: params.limit, strategy_version_id: params.strategyVersionId } : undefined }
    );
