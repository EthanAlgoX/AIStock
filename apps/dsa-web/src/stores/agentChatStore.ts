...
function streamFailureFallback(event: StreamFailureEvent, defaultMessage: string): string {
  if (event.backend === 'external_runtime') {
    ...
    try {
      ...
      const processLine = (line: string) => {
        ...
        if (event.type === 'accepted') {
          ...
          if (
            (event.backend !== 'litellm' && event.backend !== 'codex_app_server' && event.backend !== 'external_runtime')
          ...
          set((s) => ({
            ...
            serverCancellation: acceptedEvent!.backend === 'codex_app_server' || acceptedEvent!.backend === 'external_runtime',
