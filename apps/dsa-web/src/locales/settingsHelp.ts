...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.FRED_API_KEY': {
    title: 'FRED API Key',
    summary: '启用美国圣路易斯联储 FRED 官方宏观序列。',
    usage: '在 FRED 免费申请 API Key 后填入；留空时继续使用免密钥宏观来源，并明确保留缺失项。',
    valueNotes: ['覆盖美债收益率、实际利率、高收益债利差、通胀与就业等官方序列。'],
    impact: ['配置后加入宏观数据合并链，并优先于行情代理值。'],
    notes: ['密钥只保存在后端配置中，不会进入 Agent Prompt 或运行快照。'],
  },
  'settings.data_source.HITHINK_FINANCE_API_KEY': {
    title: '同花顺金融数据 API Key',
    summary: '启用同花顺官方 Financial API 的 A 股行情、日 K 和主要指数能力。',
    usage: '从官方服务获取密钥后填入；留空时不会创建该数据源实例。',
    valueNotes: ['密钥只由后端用于 X-api-key 请求头，不会进入 Agent 上下文。'],
    impact: ['配置后自动加入 A 股日 K 和默认实时行情回退链。'],
    notes: ['不要把真实密钥提交到仓库、Prompt、日志或截图。'],
  },
  'settings.data_source.HITHINK_FINANCE_BASE_URL': {
    title: '同花顺金融数据接口地址',
    summary: '设置 Financial API 的 REST 根地址。',
    usage: '通常保持官方默认地址；仅在明确使用可信兼容网关时覆盖。',
    valueNotes: ['必须是 http 或 https 地址。'],
    impact: ['覆盖后，密钥和查询内容会发送到该地址。'],
    notes: ['第三方地址会改变信任边界，请先评估安全性。'],
  },
  'settings.data_source.HITHINK_FINANCE_TIMEOUT_SECONDS': {
    title: '同花顺金融数据超时',
    summary: '限制单次 REST 请求的最长等待时间。',
    usage: '填写 0.1 至 120 秒的数值，默认 15 秒。',
    valueNotes: ['网络错误和服务端错误最多进行三次有限重试。'],
    impact: ['较大值提高慢网络成功率，也会拉长切换到下一数据源的时间。'],
    notes: ['建议先保持默认值。'],
  },
  'settings.data_source.HITHINK_FINANCE_PRIORITY': {
    title: '同花顺金融数据优先级',
    summary: '控制该来源在 A 股日 K 回退链中的尝试位置。',
    usage: '填写 0 至 99 的整数，数值越小越早尝试，默认 0。',
    valueNotes: ['未配置 API Key 时该值不生效。'],
    impact: ['只控制通用日 K 顺序；实时行情顺序由 REALTIME_SOURCE_PRIORITY 控制。'],
    notes: ['显式策略数据源选择不依赖该排序。'],
  },
  ...
  'settings.base.SCREENING_ENABLED': {
    ...
    usage: '默认开启。设为 false 可隐藏选股入口并停用选股策略。',
  ...
  'settings.notification.report_output': {
    ...
    usage: 'REPORT_TYPE 可选 simple/full/brief，REPORT_LANGUAGE 可选 zh/en/ko/ja/zh-TW；Agent Chat 只有在未显式传入 context.report_language 时才继承这里的默认语言。',
  ...
  'settings.agent.AGENT_BACKEND': {
    title: '问股生成方式',
    showFieldKey: false,
    summary: '选择主 Agent 使用现有默认模型、独立 Agent 引擎，还是运行 DSA 设备上的 Codex。',
    usage: '希望使用完整 ReAct、Skill、Tool、MCP、会话和记忆时选择独立 Agent 引擎，并配置其 API 地址；原有路径继续使用“自动（推荐）”。',
    valueNotes: [
      '“自动（推荐）”与“默认模型配置”都继续使用现有模型和 API 配置。',
      '“独立 Agent 引擎”通过受控 OpenAI-compatible API 调用独立进程，不会在网站主进程内重复实现 Agent 循环。',
  ...
  'settings.agent.AGENT_RUNTIME_API_BASE': {
    title: 'Agent 引擎 API 地址',
    summary: '连接独立 Agent 引擎提供的 OpenAI-compatible API。',
    usage: '填写 HTTP 根地址，不要追加 /v1/chat/completions。模型、Skill、Tool、MCP、记忆与安全规则继续由独立引擎管理。',
    impact: ['只在问股方式选择独立 Agent 引擎时生效。'],
    notes: ['网站只执行健康检查和 Agent 请求，不读取独立引擎的模型或 MCP 凭据。'],
  },
  'settings.agent.AGENT_RUNTIME_API_KEY': {
    title: 'Agent 引擎 API Key',
    summary: '独立 Agent 引擎 API 服务配置的可选 Bearer Token。',
    usage: '本机未启用认证时可留空；远程绑定必须在引擎侧配置 API Key，并在此填写相同值。',
    impact: ['用于网站后端到独立 Agent 引擎的请求认证。'],
  ...
  'settings.system.market_review': {
    ...
    valueNotes: [
      '支持 cn/hk/us/jp/kr/tw/gb/ca/au/in/de/fr，可用逗号组合；both 保留原五市场 cn,hk,us,jp,kr。',
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.FRED_API_KEY': {
    title: 'FRED API Key',
    summary: 'Enables official macro series from Federal Reserve Economic Data.',
    usage: 'Paste a free FRED API key. Without it, no-credential macro providers remain available and missing series stay explicit.',
    valueNotes: ['Covers Treasury yields, real yields, high-yield spread, inflation, and employment series.'],
    impact: ['Adds FRED to the macro merge route ahead of market-price proxies.'],
    notes: ['The key remains backend-only and is never included in Agent prompts or snapshots.'],
  },
  'settings.data_source.HITHINK_FINANCE_API_KEY': {
    title: 'HiThink Financial API Key',
    summary: 'Enables official HiThink A-share quotes, daily K-lines, and major-index data.',
    usage: 'Paste a key issued by the provider. Leaving it empty keeps this provider disabled.',
    valueNotes: ['The key is used by the backend in the X-api-key header and never enters Agent context.'],
    impact: ['When configured, the provider joins A-share daily and default realtime fallback chains.'],
    notes: ['Never commit or expose the key in prompts, logs, issues, or screenshots.'],
  },
  'settings.data_source.HITHINK_FINANCE_BASE_URL': {
    title: 'HiThink Financial API Base URL',
    summary: 'Sets the REST root URL for Financial API.',
    usage: 'Keep the official default unless an explicitly trusted compatible gateway is required.',
    valueNotes: ['The value must use http or https.'],
    impact: ['The API key and query content are sent to the configured endpoint.'],
    notes: ['A third-party endpoint changes the trust boundary.'],
  },
  'settings.data_source.HITHINK_FINANCE_TIMEOUT_SECONDS': {
    title: 'HiThink Financial API Timeout',
    summary: 'Limits the wait time for each REST request.',
    usage: 'Use a value between 0.1 and 120 seconds. The default is 15.',
    valueNotes: ['Network and server failures use at most three bounded attempts.'],
    impact: ['Higher values may help slow networks but delay fallback to the next provider.'],
    notes: ['Keep the default initially.'],
  },
  'settings.data_source.HITHINK_FINANCE_PRIORITY': {
    title: 'HiThink Financial API Priority',
    summary: 'Controls where the provider sits in the A-share daily fallback chain.',
    usage: 'Use an integer from 0 to 99. Lower values are tried earlier; the default is 0.',
    valueNotes: ['This has no effect when the API key is not configured.'],
    impact: ['This controls daily K-lines only; REALTIME_SOURCE_PRIORITY controls realtime order.'],
    notes: ['Explicit strategy data-source selection is independent of this order.'],
  },
  'settings.data_source.TICKFLOW_API_KEY': {
    title: 'TickFlow API Key',
  ...
  'settings.notification.report_output': {
    ...
    usage: 'REPORT_TYPE supports simple/full/brief. REPORT_LANGUAGE supports zh/en/ko/ja/zh-TW. Agent Chat inherits this default only when context.report_language is omitted.',
  ...
  'settings.agent.AGENT_BACKEND': {
    ...
    showFieldKey: false,
    summary: 'Choose the existing default model, an independent Agent engine, or Codex on the DSA device for the primary Agent.',
    usage: 'Choose Independent Agent Engine for a complete ReAct loop, Skills, Tools, MCP services, sessions, and memory; keep Auto for the existing route.',
    valueNotes: [
      'Auto (recommended) and Default model settings both keep the existing model and API route.',
      'Independent Agent Engine uses a controlled OpenAI-compatible API in a separate process instead of reimplementing its Agent loop inside the website process.',
  ...
  'settings.agent.AGENT_RUNTIME_API_BASE': {
    title: 'Agent Engine API Base URL',
    summary: 'Connects to the OpenAI-compatible API exposed by the independent Agent engine.',
    usage: 'Enter the HTTP root without /v1/chat/completions. The independent engine continues to own model, Skill, Tool, MCP, memory, and security configuration.',
    impact: ['Used only when the ask-stock method is Independent Agent Engine.'],
    notes: ['The website performs health checks and Agent requests but does not read engine provider or MCP credentials.'],
  },
  'settings.agent.AGENT_RUNTIME_API_KEY': {
    title: 'Agent Engine API Key',
    summary: 'Optional Bearer token configured by the independent Agent service.',
    usage: 'Leave empty for an unauthenticated loopback service. Remote bindings must configure an API key and use the same value here.',
    impact: ['Authenticates website backend requests to the independent Agent engine.'],
  ...
  'settings.system.market_review': {
    ...
    usage: 'MARKET_REVIEW_ENABLED toggles market review; DAILY_MARKET_CONTEXT_ENABLED is on by default and controls whether the daily market summary is injected into stock-analysis prompts and conservative guardrails; MARKET_REVIEW_REGION accepts a comma-separated region subset (for example cn,us,jp or cn,us,jp,kr), and `both` keeps the original five markets; invalid or empty values fall back to `cn`; MARKET_REVIEW_COLOR_SCHEME selects colors.',
    valueNotes: [
      'Supports cn/hk/us/jp/kr/tw/gb/ca/au/in/de/fr and comma-separated subsets; both retains the original cn,hk,us,jp,kr markets.',
...
export function getSettingsHelpContent(
  ...
  if (locale && ['ko', 'ja', 'zh-TW'].includes(locale)) {
    const source = settingsHelpZhCN[helpKey];
    return source ? localizedCopy(source, locale) : fallbackDescription ? { title: localizedCopy('配置说明', locale), summary: fallbackDescription } : null;
  }
