...
export function ScreeningCandidateEvidence({ row }: { row: Record<string, unknown> }) {
  ...
  return <details className="border-b border-border py-4">
    <summary className="cursor-pointer text-sm font-medium text-foreground">{String(row.name || row.code || row.symbol || tx("候选股票"))} {" "}{tx("· 研究依据与因子明细")}</summary>
    <div className="mt-5 grid min-w-0 gap-6 lg:grid-cols-2">
      <div className="min-w-0 space-y-5">{sections.map(([label, value]) => <section key={tx(label)}><h5 className="mb-2 text-sm font-semibold text-foreground">{tx(label)}</h5><ReportMarkdownBody content={value} /></section>)}{!sections.length && <p className="text-sm text-secondary-text">{tx("本次未返回详细研究依据。")}</p>}</div>
      <section className="min-w-0"><h5 className="mb-3 text-sm font-semibold text-foreground">{tx("因子评分")}</h5><p className="mb-4 text-xs text-secondary-text">{tx("策略内部评分，不是收益预测或获利概率。")}</p>
        {entries.length ? <dl className="space-y-4">{entries.map(([key, value]) => <div key={key}><div className="flex justify-between gap-3 text-sm"><dt>{tx(labels[key] || key)}</dt><dd className="tabular-nums">{value.toLocaleString("zh-CN", { maximumFractionDigits: 2 })}</dd></div>{value >= 0 && value <= 100 && <meter aria-label={tx("{0}评分", tx(labels[key] || key))} min={0} max={100} value={value} className="mt-1 h-2 w-full accent-primary" />}</div>)}</dl> : <p className="text-sm text-secondary-text">{tx("未返回因子明细。")}</p>}
      </section>
    </div>
  </details>;
}
