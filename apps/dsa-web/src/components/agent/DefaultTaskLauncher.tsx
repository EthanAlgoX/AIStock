...
export default function DefaultTaskLauncher({ kind, market = "CN", stock, onRunStarted }: {
  ...
  return <section aria-label={tx("一键默认方案")} className="rounded-xl border border-border bg-card px-4 py-4 md:px-5">
    ...
    {plan && <>
      <p className="mt-2 text-xs leading-5 text-secondary-text">{tx("本次按此方案新建运行，不覆盖你的手动配置。")}{kind === "trading" ? tx("仅生成模拟提案，不会自动下单。") : tx("使用真实数据与模型，可能产生调用费用。")}</p>
      <details className="mt-2 text-sm text-secondary-text"><summary className="w-fit cursor-pointer text-primary">{tx("查看选择依据、Skill 与调用成本")}</summary>
        <p className="mt-3">Skill：{plan.skillNames.map(txName => tx(txName)).join(language === "en" ? ", " : "、") || tx("正式策略综合分析方法")}{tx("；专家：")}{plan.expertCount} {" "}{tx("位。")}</p>
        <ul className="mt-2 list-disc space-y-1 pl-5">{plan.reasons.map((reason) => <li key={tx(reason)}>{tx(reason)}</li>)}</ul>
        {plan.warnings.map((warning) => <p key={tx(warning)} className="mt-2 text-warning">{tx(warning)}</p>)}
