...
export function StrategyMessageBlock({ children }: { children?: ReactNode }) {
  ...
  if (isValidElement<{ className?: string; children?: ReactNode }>(children)
    ...
    return <details className="my-3 rounded border border-border p-3">
      <summary className="cursor-pointer text-sm text-secondary-text">{(language !== 'zh' && language !== 'en') ? translateSource('策略草稿已输出 · 查看原始内容', language) : (language === 'zh' ? '策略草稿已输出 · 查看原始内容' : 'Strategy draft output · inspect original')}</summary>
