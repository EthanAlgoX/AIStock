...
export default function HoldingPlanEditor({ item, watch, onSaved }: { item?: HoldingItem; watch?: WatchItem; onSaved: () => void }) {
  ...
  const save = async (dailyEnabled: boolean) => {
    ...
    try {
      const payload = { decisionBackend: (plan.task.config.decisionBackend || 'llm') as 'llm' | 'jev', strategyVersionId: Number(plan.task.config.strategyVersionId), capabilities: bindings, dailyEnabled, dailyNotify, intervalDays: Number(intervalDays), runAt };
  ...
  return <div className="mt-6 border-t border-border bg-hover/20 p-4 md:p-6">
    ...
    <fieldset disabled={busy} className="mt-5 grid gap-6 md:grid-cols-2">
      <ResearchBackendSelect value={plan.task.config.decisionBackend === 'jev' ? 'jev' : 'llm'} watch={isWatch} onChange={decisionBackend => setPlan({ ...plan, task: { ...plan.task, config: { ...plan.task.config, decisionBackend } } })} />
      <fieldset disabled={plan.task.config.decisionBackend === 'jev'} className="contents">
