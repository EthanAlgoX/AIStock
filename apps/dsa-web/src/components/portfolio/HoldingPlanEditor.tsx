...
export default function HoldingPlanEditor({ item, watch, onSaved }: { item?: HoldingItem; watch?: WatchItem; onSaved: () => void }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (mounted) { setPlan(next); setCatalog(caps); setOptions(choices.filter(o => o.market.toUpperCase() === market.toUpperCase())); setIntervalDays(String(next.schedule?.intervalDays || 1)); setRunAt(next.schedule?.runAt || next.runAt); setCustom(next.task.capabilities.skillIds.length > 0); setError(''); }
  ...
  const save = async (dailyEnabled: boolean) => {
    ...
    try {
      const payload = { strategyVersionId: Number(plan.task.config.strategyVersionId), capabilities: bindings, dailyEnabled, intervalDays: Number(intervalDays), runAt };
