...
export default function HoldingPlanEditor({ item, onSaved }: { item: HoldingItem; onSaved: () => void }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (mounted) { setPlan(next); setCatalog(caps); setOptions(choices.filter(o => o.market.toUpperCase() === item.position.market.toUpperCase())); setIntervalDays(String(next.schedule?.intervalDays || 1)); setDaily(Boolean(next.schedule?.enabled)); setRunAt(next.schedule?.runAt || next.runAt); setCustom(next.task.capabilities.skillIds.length > 0); setError(''); }
  ...
  const save = async () => {
    ...
    try { await portfolioResearchApi.configure(item.accountId, item.position.symbol, { strategyVersionId: Number(plan.task.config.strategyVersionId), capabilities: bindings, rules, dailyEnabled: daily, intervalDays: Number(intervalDays), runAt }); onSaved(); }
  ...
  return <div className="mt-6 border-t border-border bg-hover/20 p-4 md:p-6">
    ...
    <button className="btn-primary mt-5" disabled={busy || !runAt || !Number.isInteger(Number(intervalDays)) || Number(intervalDays) < 1 || Number(intervalDays) > 365 || Object.values(rules).some(v => v < 0.1 || v > 100)} onClick={() => void save()}>{busy ? l('正在保存…', 'Saving…') : l('保存跟踪设置', 'Save tracking settings')}</button>
  </div>;
}
