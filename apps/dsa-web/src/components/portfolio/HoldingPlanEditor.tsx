...
export default function HoldingPlanEditor({ item, onSaved }: { item: HoldingItem; onSaved: () => void }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (mounted) { setPlan(next); setCatalog(caps); setOptions(choices.filter(o => o.market.toUpperCase() === item.position.market.toUpperCase())); setIntervalDays(String(next.schedule?.intervalDays || 1)); setRunAt(next.schedule?.runAt || next.runAt); setCustom(next.task.capabilities.skillIds.length > 0); setError(''); }
  ...
  const save = async (dailyEnabled: boolean) => {
    ...
    try {
      const saved = await portfolioResearchApi.configure(item.accountId, item.position.symbol, { strategyVersionId: Number(plan.task.config.strategyVersionId), capabilities: bindings, rules, dailyEnabled, intervalDays: Number(intervalDays), runAt });
      setPlan(saved);
      onSaved();
    }
  ...
  return <div className="mt-6 border-t border-border bg-hover/20 p-4 md:p-6">
    ...
    <div className="mt-5 flex flex-wrap gap-3">
      <button className="btn-primary" disabled={busy || !runAt || !Number.isInteger(Number(intervalDays)) || Number(intervalDays) < 1 || Number(intervalDays) > 365 || Object.values(rules).some(v => v < 0.1 || v > 100)} onClick={() => void save(true)}>{busy ? l('正在保存…', 'Saving…') : l('保存并开启自动跟踪', 'Save & enable tracking')}</button>
