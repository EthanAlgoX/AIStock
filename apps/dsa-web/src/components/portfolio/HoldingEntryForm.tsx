...
export default function HoldingEntryForm({ onSaved }: { onSaved: () => void }) {
  ...
  const [savedTrade, setSavedTrade] = useState<{ account: number; symbol: string } | null>(null);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      if (savedTrade) {
        await portfolioResearchApi.setBackend(savedTrade.account, savedTrade.symbol, backend);
        setSavedTrade(null); setUid(crypto.randomUUID()); setSymbol(''); setQuantity(''); setPrice(''); onSaved();
        return;
      }
      ...
      if (side === 'buy') {
        setSavedTrade({ account: id, symbol: code });
        await portfolioResearchApi.setBackend(id, code, backend);
        setSavedTrade(null);
      }
  ...
  return <form onSubmit={event => void submit(event)} className="my-5 border-y border-border bg-hover/20 p-4 md:p-6">
    ...
    {side === 'buy' && <div className="mt-5"><ResearchBackendSelect value={backend} onChange={setBackend} disabled={busy} /></div>}
    {savedTrade && <p role="status" className="mt-4 text-sm text-warning">{t('交易记录已保存，模型选择尚未保存。重试只保存模型选择，不会重复记账。')}</p>}
