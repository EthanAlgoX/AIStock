...
export default function HoldingRecommendationTrend({ history, trend, watch = false }: { history: ResearchScorePoint[]; trend: { direction: string; change: number | null; sessions: number }; watch?: boolean }) {
  ...
  const point = (entry: ResearchScorePoint, index: number) => ({
  ...
  const labelDate = (entry: ResearchScorePoint) => entry.session?.slice(5, 10) || entry.createdAt?.slice(0, 10) || '—';
  const title = watch ? l('研究评分轨迹', 'Research score trajectory') : l('持仓建议轨迹', 'Holding recommendation trajectory');
  return <section className="mt-4 max-w-[560px] border-t border-border pt-4" aria-label={title}>
    <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1"><h4 className="text-sm font-semibold text-foreground">{title}</h4><p className="text-xs text-secondary-text">{direction}{change && ` · ${change} ${l('分', 'pts')}`}</p></div>
    ...
    <svg className="mt-3 h-auto w-full" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label={l(`最近 ${history.length} 次研究的${watch ? '评分' : '持仓建议评分'}曲线`, `${watch ? 'Research score' : 'Recommendation score'} across the latest ${history.length} reports`)}>
