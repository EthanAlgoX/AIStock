...
export default function WatchEntryForm({ onSaved }: { onSaved: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!(market === 'crypto' ? /^[A-Z0-9]{2,16}USDT$/.test(code) : market === 'cn' ? /^\d{6}$/.test(code) : market === 'hk' ? /^HK\d{5}$/.test(code) : /^[A-Z][A-Z0-9.-]{0,14}$/.test(code) && !/^HK\d+$/.test(code))) throw new Error(l('请输入匹配市场的股票代码，或从列表中选择。', 'Enter a stock code matching the market, or choose from the list.'));
  ...
  return <form onSubmit={event => void submit(event)} className="my-5 border-y border-border bg-hover/20 p-4 md:p-6">
    ...
    <fieldset disabled={busy} className="mt-5 grid gap-4 sm:grid-cols-2">
      <label className="text-sm">{l('股票市场', 'Stock market')}<select value={market} onChange={e => { setMarket(e.target.value as typeof market); setSymbol(''); }} className={input}><option value="cn">{l('A 股', 'China A')}</option><option value="hk">{l('港股', 'Hong Kong')}</option><option value="us">{l('美股', 'US')}</option><option value="crypto">{l("加密货币", "Crypto")} · USDT</option></select></label>
      <label className="text-sm">{l('股票代码或名称', 'Stock code or name')}<input required list="watch-symbols" value={symbol} onChange={e => setSymbol(e.target.value)} placeholder={{ cn: '600519', hk: 'HK00700', us: 'AAPL', crypto: 'BTCUSDT' }[market]} className={input} /><datalist id="watch-symbols">{index.filter(stock => stock.market.toLowerCase() === market && `${stock.canonicalCode} ${stock.nameZh}`.toLowerCase().includes(symbol.toLowerCase())).slice(0, 20).map(stock => <option key={stock.canonicalCode} value={stock.canonicalCode}>{stock.nameZh}</option>)}</datalist></label>
