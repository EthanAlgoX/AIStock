...
export function ChartCodeBlock({children}:{children?:ReactNode}) {
  if(isValidElement<{className?:string;children?:ReactNode}>(children)&&children.props.className==='language-analysis-chart') {
    ...
    return <details className="not-prose my-4 border border-border p-3"><summary className="cursor-pointer text-sm text-secondary-text"><UiLiteral text={"图表数据尚未完整或格式无效 · 查看原文 / Chart data incomplete or invalid"} /></summary><pre className="mt-3 overflow-auto whitespace-pre-wrap text-xs">{raw}</pre></details>;
  }
