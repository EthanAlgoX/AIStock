...
export function ResearchMemo({ report, provenance }: { report: AnalysisReport; provenance: ReactNode }) {
  ...
  ].map(([label, value], index) => ({ label, value, index })).filter(({ value }) => words(value));
  ...
  return <article className="research-memo min-w-0 text-foreground [&_.home-markdown-prose]:text-secondary-text [&_.home-markdown-prose_p]:leading-7" aria-label={l('投研备忘录', 'Research memorandum')}>
    ...
    <nav aria-label={l('报告阅读目录', 'Report contents')} className="flex flex-wrap gap-x-6 border-t border-border text-sm">
      {[["conclusion", l('研究结论', 'Conclusion')], ["conditions", l('观察与行动条件', 'Conditions')], ["evidence", l('证据对照', 'Evidence')], ["sources", l('来源与运行记录', 'Sources and run record')]].map(([key, label]) => <a key={key} href={`#${id}-${key}`} className="inline-flex min-h-11 items-center text-secondary-text hover:text-primary focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary">{label}</a>)}
    </nav>
    <div className="grid min-w-0 gap-x-8">
      <div className="min-w-0">
        <section id={`${id}-conclusion`} className={sectionClass}>
          ...
          <h3 className="text-balance text-xl font-semibold leading-snug tracking-tight text-foreground sm:text-2xl">{thesis || summary.operationAdvice || l('研究摘要', 'Research summary')}</h3>
        ...
        <section id={`${id}-conditions`} className={sectionClass}>
          ...
          {levels.length ? <dl className="mt-5 grid gap-4 md:grid-cols-2">{levels.map(({ label, value, index }) => {
            const Icon = conditionIcons[index] || Crosshair;
            const tone = index === 2 ? 'text-danger' : index === 3 ? 'text-success' : 'text-primary';
            return <div key={label} className="min-w-0 rounded-xl border border-border p-5"><dt className={`mb-3 flex items-center gap-2 text-sm font-medium ${tone}`}><Icon size={18} aria-hidden="true" />{label}</dt><dd className="min-w-0 text-base leading-7 text-foreground [&_.home-markdown-prose]:text-foreground"><ReportMarkdownBody content={value!} /></dd></div>;
          })}</dl> : <p className="mt-5 text-sm text-secondary-text">{l('本报告未提供价格条件，不补造点位。', 'No price conditions were recorded; none are inferred.')}</p>}
        ...
        <section id={`${id}-evidence`} className={sectionClass}>
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-2"><section className="min-w-0 rounded-xl bg-success/5 p-4 sm:p-5"><h4 className="mb-4 flex items-center justify-between gap-3 text-sm font-semibold"><span>{l('正向信号与催化', 'Positive signals and catalysts')}</span><span className="text-secondary-text">{support.length}</span></h4>{evidenceList(support)}</section><section className="min-w-0 rounded-xl bg-warning/5 p-4 sm:p-5"><h4 className="mb-4 flex items-center justify-between gap-3 text-sm font-semibold"><span>{l('负向信号与风险', 'Negative signals and risks')}</span><span className="text-secondary-text">{challenges.length}</span></h4>{evidenceList(challenges)}</section></div>
        </section>
      ...
      <aside id={`${id}-boundaries`} aria-label={l('研究边界', 'Research boundaries')} className="min-w-0 scroll-mt-24 border-t border-border py-7">
