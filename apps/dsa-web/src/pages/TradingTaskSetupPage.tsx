    ...
    <div className="space-y-6 pb-6" data-testid="trading-strategy-workspace">
      <ol className="grid gap-px overflow-hidden rounded-[12px] border border-border bg-border sm:grid-cols-4" aria-label="交易策略配置流程">
        {[
          ...
          { label: "生成提案", ready: Boolean(activeRun) },
        ].map((step, index) => (
          ...
          <div className="overflow-hidden rounded-[14px] border border-border bg-card shadow-soft-card">
            ...
            <section className="px-5 py-5 sm:px-6" aria-labelledby="trading-capability-heading">
              ...
              <div className="mt-5 flex flex-col gap-3 border-t border-border/70 pt-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap gap-2"><button type="button" className="btn-secondary inline-flex items-center gap-2" onClick={() => void saveDraft()}><Save className="h-4 w-4" />保存交易策略</button><button type="button" disabled={!strategyReady} className="btn-secondary inline-flex items-center gap-2 disabled:cursor-not-allowed disabled:opacity-45" onClick={() => void scheduleStrategy()}><CalendarClock className="h-4 w-4" />创建定时计划</button><button type="button" disabled={!strategyReady || busy} className="btn-primary inline-flex items-center gap-2 disabled:cursor-not-allowed disabled:opacity-45" onClick={() => void startRun()}><Play className="h-4 w-4" />{restoring ? "恢复运行状态…" : submitting ? "正在提交…" : "启动模拟运行"}</button></div>
              </div>
