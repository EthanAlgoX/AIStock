...
const time = (value: string, language: string) => {
  ...
  return Number.isNaN(date.getTime()) ? "—" : date.toLocaleString(language === "en" ? "en-US" : "zh-CN", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" });
};
...
function RunElapsed({ run }: { run: WorkspaceRun }) {
  ...
  return <span className="tabular-nums">{Number.isFinite(seconds) ? [Math.floor(seconds / 3600), Math.floor(seconds / 60) % 60, seconds % 60].map((part) => String(part).padStart(2, "0")).join(":") : tx("未提供")}</span>;
}
