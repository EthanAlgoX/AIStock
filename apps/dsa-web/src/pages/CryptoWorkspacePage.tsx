...
export default function CryptoWorkspacePage() {
  ...
  const availablePairs = useMemo(() => [...new Set([...PAIRS, ...market.map(row => row.symbol)])], [market]);
  ...
  const runBacktest = async () => { setBusy('backtest'); setError(''); try { setBacktest(await cryptoApi.backtest({ symbols, strategy, startDate, endDate, initialCash: 10000, feeRate: .001, slippageRate: .0005, allocation: .5, lookbackHours: 720, topN: Math.min(3, symbols.length), rebalanceHours: 168 })); } catch { setError(l('回测失败：请核对日期与交易对，并确保有完整的历史小时线。', 'Backtest failed: check the dates and pairs, and ensure complete historical hourly candles are available.')); } finally { setBusy(''); } };
  ...
  return <AppPage>
    <PageHeader eyebrow="Binance Spot · USDT" title={l('加密货币研究与回测', 'Crypto research and backtesting')} description={l('查看现货行情，按过去一个月的成交额和波动率选币，并用固定规则回测。所有交易仅为模拟，不会连接交易账户。', 'Explore spot prices, screen by past-month turnover and volatility, and replay fixed rules. Trades are simulated and never connect to an exchange account.')} />
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-2">
      <section className="rounded-xl border border-border bg-card p-5">
        ...
        {screen && <div className="mt-4 text-sm"><p className="font-semibold">{l('选中：', 'Selected: ')}{screen.selected}</p><p className="mt-1 text-xs text-secondary-text">{l('信号截止：', 'Signal cutoff: ')}{new Date(screen.signalTime).toLocaleString(language)}</p><ol className="mt-2 space-y-1">{screen.ranking.map(row => <li key={row.symbol}>{row.symbol} · {l('成交额', 'turnover')} {number(row.quoteVolume, 0)} USDT · {l('波动率', 'volatility')} {number(row.volatility * 100, 3)}%</li>)}</ol></div>}
      </section>
      <section className="rounded-xl border border-border bg-card p-5">
    ...
    {backtest && <section className="mt-5 rounded-xl border border-border bg-card p-5">
      <h2 className="text-lg font-semibold">{l('模拟持仓与成交', 'Simulated positions and fills')}</h2>
      ...
      <p className="mt-3 text-sm">USDT {l('可用现金', 'cash available')}: {number(backtest.endingCash)}</p>
      <div className="mt-2 flex flex-wrap gap-3 text-sm">{Object.entries(backtest.endingPositions).filter(([, quantity]) => quantity > 1e-8).map(([pair, quantity]) => <span className="rounded-lg border border-border px-3 py-2" key={pair}>{pair}: {number(quantity, 8)}</span>)}</div>
      <div className="mt-4 max-h-56 overflow-auto text-sm">{backtest.trades.length ? backtest.trades.map((trade, i) => <p className="border-t border-border/60 py-2" key={`${trade.time}-${trade.symbol}-${i}`}>{new Date(trade.time).toLocaleString(language)} · {trade.side === 'buy' ? l('买入', 'Buy') : l('卖出', 'Sell')} {trade.symbol} · {number(trade.quantity, 8)} @ {number(trade.price, 6)} · {l('手续费', 'Fee')} {number(trade.fee, 4)} USDT</p>) : <p>{l('没有成交', 'No fills')}</p>}</div>
    </section>}
  </AppPage>;
}
