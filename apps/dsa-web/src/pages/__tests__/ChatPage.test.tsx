...
describe('ChatPage', () => {
  ...
  it('lets the user stop an active main Agent analysis without exposing its runtime', async () => {
    ...
    expect(screen.queryByText(/Nanobot Runtime/i)).not.toBeInTheDocument();
  });
  ...
  it('keeps the main Agent page product-focused when Nanobot is active', async () => {
    mockGetStatus.mockResolvedValueOnce({
      backend: 'nanobot',
      available: true,
      experimental: false,
    ...
    expect(screen.queryByText(/Nanobot/i)).not.toBeInTheDocument();
    expect(screen.getByPlaceholderText(/分析 600519/)).toBeEnabled();
  });
