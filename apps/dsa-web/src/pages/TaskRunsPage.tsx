...
export default function TaskRunsPage() {
  ...
  return (
    <AppPage className="space-y-6 pb-20" data-testid="task-runs-page">
      ...
      <section className="grid gap-px overflow-hidden rounded-[12px] border border-border bg-border sm:grid-cols-3" aria-label={uiLiteral("任务运行状态摘要")}>
        <div className="bg-card px-5 py-4"><p className="text-xs text-secondary-text"><UiLiteral text={"已注册计划"} /></p><p className="mt-2 font-mono text-2xl font-semibold text-foreground">{loading ? "—" : schedules.length}</p><p className="mt-1 text-xs text-muted-text"><UiLiteral text={"后端持久化调度"} /></p></div>
        <div className="bg-card px-5 py-4"><p className="text-xs text-secondary-text"><UiLiteral text={"匹配运行"} /></p><p className="mt-2 font-mono text-2xl font-semibold text-foreground">{loading ? "—" : total}</p><p className="mt-1 text-xs text-muted-text"><UiLiteral text={"手动与定时运行"} /></p></div>
        <div className="bg-card px-5 py-4"><p className="text-xs text-secondary-text"><UiLiteral text={"正在运行"} /></p><p className="mt-2 font-mono text-2xl font-semibold text-foreground">{loading ? "—" : (counts.running || 0) + (counts.queued || 0)}</p><p className="mt-1 text-xs text-muted-text"><UiLiteral text={"当前筛选范围内的排队与执行任务"} /></p></div>
      </section>
      ...
      <section className="overflow-hidden rounded-[14px] border border-border bg-card shadow-soft-card" aria-labelledby="run-ledger-heading">
        ...
        {runs.length ? <div className="divide-y divide-border/60">{runs.map((run) => {
          ...
          return <article key={run.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[2.5rem_minmax(0,1fr)_11rem_9rem] sm:items-center">
            ...
            <div><p className="text-sm font-semibold text-foreground">{run.taskSnapshot?.name || uiLiteral(meta.label)}</p><p className="mt-1 text-xs text-secondary-text">{uiLiteral(meta.label)} · {run.taskSnapshot?.market || "—"}</p>{run.errorMessage ? <p className="mt-1 line-clamp-1 text-xs text-danger">{uiLiteral(run.errorMessage)}</p> : null}</div>
        ...
        })}</div> : <div className="flex min-h-56 flex-col items-center justify-center px-6 py-10 text-center"><CalendarClock className="h-7 w-7 text-muted-text" /><p className="mt-4 font-medium text-foreground"><UiLiteral text={"没有匹配的运行记录"} /></p><p className="mt-2 max-w-lg text-sm leading-6 text-secondary-text"><UiLiteral text={"从个股分析、选股、交易或专家评审页面启动第一个任务。"} /></p><Link to="/stock-research" className="mt-5 inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:underline"><UiLiteral text={"开始个股分析 "} /><ArrowRight className="h-3.5 w-3.5" /></Link></div>}
      </section>

      <nav aria-label={uiLiteral("运行历史分页")} className="flex items-center justify-between gap-3 text-sm"><button className="btn-secondary" disabled={offset === 0} onClick={() => { const next = new URLSearchParams(params); next.set("offset", String(Math.max(0, offset - 30))); setParams(next); }}><UiLiteral text={"上一页"} /></button><span><UiLiteral text={"共 "} />{total} <UiLiteral text={" 条 · 第 "} />{Math.floor(offset / 30) + 1} <UiLiteral text={" 页"} /></span><button className="btn-secondary" disabled={offset + 30 >= total} onClick={() => { const next = new URLSearchParams(params); next.set("offset", String(offset + 30)); setParams(next); }}><UiLiteral text={"下一页"} /></button></nav>
      <section className="overflow-hidden rounded-[12px] border border-border bg-background" aria-labelledby="run-contract-heading">
        <div className="border-b border-border px-5 py-4"><div className="flex items-center gap-2"><FileCheck2 className="h-4 w-4 text-primary" /><h2 id="run-contract-heading" className="font-semibold text-foreground"><UiLiteral text={"每次运行保留什么"} /></h2></div></div>
        <ol className="grid gap-px bg-border sm:grid-cols-4">{["提交时的目标与配置", "使用的工具与专家", "数据来源与时间", "报告与失败原因"].map((item, index) => <li key={item} className="bg-card px-5 py-4"><span className="font-mono text-[10px] text-muted-text">0{index + 1}</span><p className="mt-2 text-xs font-medium text-foreground">{uiLiteral(item)}</p></li>)}</ol>
      </section>
    </AppPage>
  );
}
