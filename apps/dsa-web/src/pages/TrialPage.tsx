  ...
  return <main className="min-h-screen bg-background px-5 py-7 text-foreground md:px-10">
    <div className="mx-auto max-w-6xl">
      ...
      <section id="live-trial" className="border-t border-border py-8">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-semibold">{l('限额真实试用', 'Live research trial')}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-secondary-text">{l('每个受邀用户使用独立每日额度，管理员可随时调整；输入、输出及专家调用合并计算。受限研究不等于完整工作流：不访问个人数据、不扫描全市场、不运行回测、不下单。', 'Each invited user has an adjustable daily token limit shared across input, output and experts. Limited research is not the full workflow: no personal data, market-wide scans, backtests or orders.')}</p></div>{user && <button className="btn-secondary" onClick={async () => { try { await trialApi.logout(); resetIdentityView(); } catch (err) { setError(err); } }}>{l('退出试用账户', 'Sign out of trial')}</button>}</div>
        ...
        {!user ? <details className="mt-6 max-w-lg rounded-xl border border-border p-5"><summary className="cursor-pointer font-medium">{l('领取邀请码额度 / 试用登录', 'Claim an invitation / trial sign in')}</summary><p className="mt-3 text-sm leading-6 text-secondary-text">{l('邀请码由部署管理员发放，领取时自行填写登录邮箱。它不是管理员初始化凭证；没有邀请码也可以继续浏览上方演示。', 'Invites are issued by the deployment administrator, and you choose your login email when claiming one. They are not administrator setup tokens; without one, you can still explore the demo above.')}</p>
          ...
          <div className="my-6 flex flex-wrap gap-x-8 gap-y-2 rounded-lg bg-hover px-4 py-3 text-sm"><span className="break-all">{user.email}</span><strong className="tabular-nums">{l('剩余', 'Remaining')} {user.remaining.toLocaleString()} / {user.limit.toLocaleString()}</strong><span className="text-secondary-text">{l('每日 UTC 00:00（北京时间 08:00）重置', 'Resets daily at 00:00 UTC')}</span></div>
