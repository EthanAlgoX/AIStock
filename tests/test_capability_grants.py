...
def test_tampered_gateway_grant_is_rejected() -> None:
    policy = build_runtime_capability_policy(
        {"runtimePolicy": {"gatewayToolIds": [], "nanobotToolIds": []}},
        session_id="session-1",
        configured_secret="test-secret",
    )
...
def test_nanobot_mcp_name_matches_runtime_limit_contract() -> None:
    short = nanobot_mcp_tool_name("finance", "get_realtime_quote")
    long = nanobot_mcp_tool_name("finance", "x" * 100)
