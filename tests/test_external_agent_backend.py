# -*- coding: utf-8 -*-
"""External Agent sidecar adapter contract tests."""
...
from src.agent.external_agent_backend import (
    ExternalAgentBackend,
    ExternalAgentHTTPTransport,
    ExternalAgentTransportError,
    normalize_agent_runtime_api_base,
)
...
def test_normalize_agent_runtime_api_base_accepts_root_or_v1() -> None:
    assert normalize_agent_runtime_api_base("http://127.0.0.1:8900/") == "http://127.0.0.1:8900"
    assert normalize_agent_runtime_api_base("https://agent.example/v1") == "https://agent.example"
...
def test_normalize_agent_runtime_api_base_rejects_unsafe_or_invalid_values(value: str) -> None:
    with pytest.raises(ExternalAgentTransportError) as exc_info:
        normalize_agent_runtime_api_base(value)
...
def test_http_transport_uses_official_api_and_isolates_site_session_id() -> None:
    ...
    class FakeSession:
        def get(self, url, **kwargs):
            ...
            return FakeResponse({"data": [{"id": "external_runtime-agent"}]})

        def post(self, url, **kwargs):
            calls.append(("post", url, kwargs))
            return FakeResponse(
                {
                    "model": "external_runtime-agent",
...
def test_external_runtime_backend_delegates_loop_and_preserves_site_contract() -> None:
    calls = []
    events = []
    ...
    backend = ExternalAgentBackend(
        SimpleNamespace(
            agent_runtime_api_base="http://external_runtime.internal",
            agent_runtime_api_key="secret",
    ...
    assert result.backend == "external_runtime"
    assert result.final_answer == "结构化研究结论"
    assert result.usage["total_tokens"] == 20
    assert calls[0] == ("http://external_runtime.internal", "secret")
    ...
    assert all("Independent Agent Engine" not in event.get("message", "") for event in events)


def test_external_runtime_backend_maps_transport_failure_without_secret_disclosure() -> None:
    class BrokenTransport:
        def __init__(self, *_args):
            pass

        def chat(self, *_args, **_kwargs):
            raise ExternalAgentTransportError("authentication_failed", "internal secret detail")

    backend = ExternalAgentBackend(
        SimpleNamespace(agent_runtime_api_base="http://external_runtime.internal", agent_runtime_api_key="secret"),
...
def test_external_runtime_backend_fails_closed_without_capability_manifest() -> None:
    backend = ExternalAgentBackend(
        SimpleNamespace(agent_runtime_api_base="http://external_runtime.internal", agent_runtime_api_key=""),
...
def test_external_runtime_probe_rejects_runtime_without_capability_isolation() -> None:
...
def test_external_runtime_transport_honors_cancellation_before_request() -> None:
...
def test_external_runtime_transport_maps_interrupted_stream_to_cancelled() -> None:
    cancel_event = threading.Event()
...
def test_external_runtime_transport_interrupts_blocking_socket_on_cancellation() -> None:
    cancel_event = threading.Event()
...
def test_external_runtime_preparation_uses_runtime_capabilities_instead_of_dsa_tool_contract() -> None:
    backend = ExternalAgentBackend(
        SimpleNamespace(agent_runtime_api_base="http://external_runtime.internal", agent_runtime_api_key=""),
    )
