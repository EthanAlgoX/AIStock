...
class ScreeningOpportunitiesApiTestCase(unittest.TestCase):
    ...
    def test_start_screen_task_submits_background_work(self) -> None:
        ...
        self.assertIsNone(payload.strategy_version_id)
        self.assertEqual(payload.created_at, "2026-08-18T09:00:00")
        ...
        self.assertIsNone(fake_queue.submit_background_task.call_args.kwargs["strategy_version_id"])
    ...
    def test_start_screen_task_persists_selected_strategy_version(self) -> None:
        config = self._config(enabled=True)
        fake_queue = MagicMock()
        database = MagicMock()
        fake_queue.submit_background_task.return_value = SimpleNamespace(
            task_id="screen-task-versioned",
            trace_id="screen-task-versioned",
            status=QueueTaskStatus.PENDING,
            message="选股任务已提交",
            created_at=datetime(2026, 8, 18, 9, 5, 0),
        )
        fake_queue.get_task.return_value = SimpleNamespace(
            created_at=datetime(2026, 8, 18, 9, 5, 0),
            started_at=datetime(2026, 8, 18, 9, 5, 1),
        )

        with (
            patch("api.v1.endpoints.screening.get_task_queue", return_value=fake_queue),
            patch("api.v1.endpoints.screening._validate_product_version") as validate_version,
            patch.object(
                screening_endpoint.StrategyKernelExecutorService,
                "execute_product",
                return_value={
                    "status": "success",
                    "contract": "CandidateList",
                    "result": {"run_id": "persisted-screen-run", "strategy": "dual_low", "market": "cn", "candidate_count": 0},
                },
            ),
        ):
            payload = screening_endpoint.screening_start_screen_task(
                screening_endpoint.ScreeningScreenRequest(
                    market="cn",
                    strategy="dual_low",
                    max_results=3,
                    strategy_version_id=57,
                ),
                http_request=self._request(),
                config=config,
                db_manager=database,
        ...
        validate_version.assert_called_once_with(57)
        self.assertEqual(payload.strategy_version_id, 57)
        self.assertEqual(payload.created_at, "2026-08-18T09:05:00")
        self.assertEqual(fake_queue.submit_background_task.call_args.kwargs["strategy_version_id"], 57)
        database.save_screening_run.assert_called_once()
        self.assertEqual(database.save_screening_run.call_args.args[0]["strategy_version_id"], 57)
        self.assertEqual(database.save_screening_run.call_args.args[0]["run_id"], "persisted-screen-run")
    ...
    def test_screen_task_status_returns_screening_result(self) -> None:
        task = TaskInfo(
            ...
            strategy_version_id=57,
            created_at=datetime(2026, 8, 18, 9, 0, 0),
            started_at=datetime(2026, 8, 18, 9, 0, 1),
            completed_at=datetime(2026, 8, 18, 9, 1, 0),
        )
        fake_queue = MagicMock()
