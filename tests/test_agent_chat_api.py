...
def _external_runtime_config(**overrides):
    values = {
...
def test_agent_models_do_not_fall_back_to_litellm_for_codex_or_invalid_backend() -> None:
    ...
    for config in (
        ...
        SimpleNamespace(agent_backend="external_runtime", agent_arch="single"),
    ):
...
def test_external_runtime_stream_forwards_server_cancellation_event() -> None:
    executor = _executor(_result(backend="external_runtime"))
    with patch("api.v1.endpoints.agent.asyncio.to_thread", side_effect=_immediate_to_thread), \
         patch("api.v1.endpoints.agent.get_config", return_value=_external_runtime_config()), \
         patch("api.v1.endpoints.agent._build_executor", return_value=executor):
        events = asyncio.run(
            _collect_stream_events(
                agent_endpoint.ChatRequest(
                    message="分析 AAPL",
                    session_id="external_runtime-cancellable",
                    request_id="external_runtime-request",
                )
            )
        )

    assert [event["type"] for event in events] == ["accepted", "done"]
    assert events[0]["backend"] == "external_runtime"
