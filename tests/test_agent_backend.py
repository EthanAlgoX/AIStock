...
def test_nanobot_agent_backend_is_explicit_opt_in() -> None:
    assert resolve_agent_backend_id(SimpleNamespace(agent_backend="nanobot")) == "nanobot"
...
def test_nanobot_chat_factory_does_not_construct_litellm_context_adapter() -> None:
    prompt_state = SimpleNamespace(
    ...
    assert executor.context_llm_adapter is None
    assert executor.backend.backend_id == "nanobot"
