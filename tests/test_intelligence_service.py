...
class IntelligenceServiceTestCase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._environment_keys = [
            "DATABASE_PATH", "NEWS_INTEL_RETENTION_DAYS", "NEWS_INTEL_MAX_ITEMS_PER_SOURCE",
            "NEWS_INTEL_FETCH_TIMEOUT_SEC", "NEWS_INTEL_AUTO_FETCH_ENABLED",
        ]
        self._environment_before = {key: os.environ.get(key) for key in self._environment_keys}
    ...
    def test_fetch_enabled_sources_is_fail_open(self) -> None:
        ...
        # This test is about batch fail-open aggregation.  DNS guard and proxy
        # propagation have dedicated tests, so inject at the service boundary
        # instead of temporarily swapping process-global socket/getaddrinfo.
        # The latter made this isolated contract dependent on unrelated tests'
        # cleanup order in a full suite.
        with patch.object(self.service, "_validate_url"), patch.object(
            self.service, "_get_with_validated_dns", side_effect=fake_get
        ):
