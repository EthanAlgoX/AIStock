...
| `AGENT_BACKEND` | 现有问股 Chat 的运行方式：`auto`（推荐）、`litellm`、`nanobot`（独立 Agent Runtime）或 `codex_app_server`（实验） | `auto` | 否 |
| `NANOBOT_API_BASE` | `nanobot serve` 的 HTTP 根地址，例如 `http://127.0.0.1:8900` | - | 使用 Nanobot 时必填 |
| `NANOBOT_API_KEY` | Nanobot API 的可选 Bearer Token；公开绑定时必须在 Nanobot 侧配置并保持一致 | - | 可选 |
...
> `AGENT_BACKEND=nanobot` 会通过独立 `nanobot serve` 委托问股 Agent 循环；模型、运行时 Skill、Tool、MCP 和记忆仍在 Nanobot 内配置。网站只保存 Nanobot API 地址和可选 Bearer Token，并保留股票范围、可见会话与交易安全边界。当前网站 MCP 草稿和数据源目录不会自动同步到 Nanobot。详见 [Nanobot 主 Agent Runtime 接入](nanobot-integration.md)。
