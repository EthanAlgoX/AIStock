...
> AI Stock 当前入口与运行方式已扩展，见 [持仓价格告警与通知](holding-price-alerts.md)。Web/API 使用独立告警 poller，不再要求开启每日研究 schedule；CLI `--schedule` 仍复用原 worker。下文 P0–P8 是继承模块的分阶段记录，“仅 schedule”“notification_policy 不执行”等历史描述不适用于新版 Web 的价格简报规则。
