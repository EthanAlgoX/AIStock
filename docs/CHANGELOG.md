...
- [新功能] Nanobot 接入升级为逐任务强隔离：网站签发短时能力授权，Runtime 为每轮构造精确 Tool/Skill 受限视图，金融 MCP 再校验签名、Tool、股票范围和数据源类别；旧 Runtime 或缺失授权一律 fail closed。
...
- [新功能] 新增受控金融 MCP HTTP Surface 与 Runtime Manifest；工作区启用的 READ / COMPUTE Tool 可发布给外部 Nanobot，健康的 HTTP MCP 可按任务动态挂载，凭据只保存环境变量名，stdio 命令不由网站执行。
...
- [改进] Skill 配置明确收敛为完整 Agent 能力中的金融白名单，只向股票研究、选股、组合风险和交易任务发布相关方法；Nanobot 式会话、记忆、工具循环、任务恢复与安全边界继续作为底层 Agent 基线，非金融通用 Skill 不进入默认网站目录。
...
- [新功能] 问股首页新增可选 Nanobot 主 Agent Runtime：通过独立 `nanobot serve` 的官方 OpenAI-compatible API 委托 ReAct、Skill、Tool、MCP、会话与记忆，网站保留股票范围、可见会话和交易安全边界；设置页新增地址、可选 API Key 与只读健康检查，默认 Agent 路径不变。
- [改进] Nanobot 启用后，`/overview` 继续只展示统一的“主 Agent”产品界面，不再在标题、空状态、消息徽标或会话能力面板重复暴露 Runtime 名称；具体后端、连接状态和能力边界集中保留在 Agent 设置页。
- [修复] 主 Agent 首页为 Nanobot 使用独立的运行时能力提示与 SSE 适配，不再等待网站内部固定工具；运行中展示阶段和真实耗时，停止操作会中断底层连接并以已取消终态结束，默认部署建议将运行时限设置为 300 秒。
