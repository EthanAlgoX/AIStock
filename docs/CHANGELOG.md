...
- [新功能] 市场情报新增 A 股、港股、美股专属宏观传导链和 15 项共同宏观检查表；市场复盘保存 Yahoo Finance 高频行情、ApocData 中国基础发布值及可选 FRED 官方序列，未接入的低频指标明确显示为待接入。
- [新功能] 数据源目录为每个实际提供方声明官方注册或接入链接、真实认证形式和对应配置键；数据源页面可从说明图标跳转官网，并通过现有受保护配置表单只编辑该适配器声明的字段。
- [改进] 自定义数据源支持登记官方接入文档与 API Key、Token、实例地址、账号授权、免密钥或自定义认证形式；服务端拒绝非 HTTP(S) 及包含账号密码的链接，数据源目录继续不保存凭据值。
- [新功能] 接入同花顺官方 Financial API 作为可配置的默认 A 股行情数据源，支持快照、前复权日 K、主要指数与现有回退链。
- [新功能] 数据源页面新增持久化单源健康检测，将“系统托管 / 已配置 / 未配置 / 已登记”与“未检测 / 可用 / 部分可用 / 不可用”分开显示，并记录检测时间、耗时、有效记录数和脱敏错误；检测具备 20 秒上限和并发保护。
- [修复] 未接入执行适配器的系统情绪占位来源不再进入 Agent 可绑定数据源，任务能力面板也不再把仅完成配置但未经检测的数据源标为“已接通”。
- [修复] YFinance 日线适配器将平台的包含式结束日期转换为 Yahoo 的排除式结束日期，避免检测与正式历史请求静默遗漏区间最后一个交易日。

- [改进] 市场情报按 A 股、港股和美股隔离读取真实已保存复盘，新增后台生成最新复盘与任务进度、近 14 天真实资讯、资讯源同步状态、数据过期提示和五类可配置展示模块；数据源“已配置”不再冒充运行健康或本次快照来源。
- [文档] README 增加默认中文与英文版本切换，以新的项目主视觉、完整策略生命周期图和端到端研究工作流重构首页，并将英文首页完整同步为当前策略内核、版本验证、运行监控和真实能力边界说明。
- [新功能] 独立 Agent 引擎 接入升级为逐任务强隔离：网站签发短时能力授权，Runtime 为每轮构造精确 Tool/Skill 受限视图，金融 MCP 再校验签名、Tool、股票范围和数据源类别；旧 Runtime 或缺失授权一律 fail closed。
- [修复] Agent-first 网站审查修正主 Agent 完成徽标仍判断旧 `/chat` 路由、任务草稿误保留未健康 MCP、能力面板失败文案、首页标题和旧 API 产品标识；数据源配置与市场情报统一改用 Workspace 能力 API，不再依赖旧上传策略命名空间。
- [chore] 恢复后端门禁与测试脚本的可执行权限，使本地验证和 GitHub Actions 可按仓库文档直接运行。
- [修复] 升级 Axios、React Router 及受影响的构建依赖到兼容的安全修复版本，清除 Web 生产与开发依赖审计中的全部已知漏洞。
- [新功能] 新增 Agent-first 工作区后端控制面：持久化 Skill、Tool 白名单、HTTP MCP、数据源、专家、专家团、Task、DataSnapshot、Run、Artifact 与 Schedule，并为研究、选股、专家评审和模拟交易提供统一 API、运行取消和服务重启状态收敛。
- [新功能] 新增受控金融 MCP HTTP Surface 与 Runtime Manifest；工作区启用的 READ / COMPUTE Tool 可发布给外部 独立 Agent 引擎，健康的 HTTP MCP 可按任务动态挂载，凭据只保存环境变量名，stdio 命令不由网站执行。
- [新功能] Agent 新增 `screen_stock_universe` 金融 Tool，复用现有股票池筛选管线生成真实候选；选股任务明确要求先扫描股票池，再输出 `ScreenSpec` 与 `CandidateList`。
- [修复] 主 Agent、个股分析、选股、交易和专家评审新建任务默认挂载经工作区白名单过滤的安全金融能力；移除浏览器本地 Skill、MCP 和专家注册表，防止前端显示状态与后端真实可用范围漂移。
- [修复] MCP 只有启用且通过 HTTP 能力发现后才能被任务选择；连接地址变化会清空旧能力并要求重新探测，定时计划也会拒绝停用任务和非法时间配置。
- [修复] 分析任务状态接口兼容队列中尚未写入开始或完成时间的任务对象，避免市场情报和个股分析轮询因可选时间字段缺失而返回 500。
- [改进] 交易 Run 只保存 `TradeProposal`、配置合同检查和未执行的 `PaperTradingRun`，明确不把配置校验描述成账户级风险通过，也不伪造成交、收益或真实订单。
- [改进] Web 应用外壳由 16 项固定左侧栏调整为顶部任务导航：桌面端只平铺主 Agent、市场情报、个股分析、选股和交易五个核心工作台，专家评审、定时任务、运行记录与能力中心收敛为辅助入口，移动端使用五项底部导航与工作区设置抽屉；能力中心和任务中心继续通过页面内导航访问独立子页。
- [改进] Web 信息架构收敛为“决策工作台 / 协作与自动化 / 能力中心 / 治理与记录”四组：新增能力总览和独立“任务与运行”账本，模型用量回归独立语义，个股分析、选股、交易与定时计划统一支持 Skill、内置工具、MCP、数据源和专家绑定；清理不再路由的上传策略、版本回测、旧组合/信号/告警/新闻页面及其孤立前端测试，历史 URL 保留到新工作台的兼容跳转。

- [改进] Web 能力配置将内置 Tool 与 MCP Server 拆分：新增独立金融工具页面及工作区白名单，MCP 页面改为服务连接语义，导航、任务能力面板和说明文档同步明确 Skill、Tool、MCP、数据源与专家的不同职责。

- [新功能] Web 新增独立“定时任务”工作台，可为单股分析和选股保存每日固定时间计划，并为交易策略选择每日或间隔运行；页面按 `ResearchReport`、`CandidateList + ScreenSpec`、`PaperTradingRun + TradeProposal` 区分三类输出，并将计划保存到后端持久化调度器。
- [改进] 定时任务采用“业务页创建、调度页管理”：个股分析、选股和交易页新增上下文式定时入口，将当前市场、股票或策略、业务条件和 Agent 能力选择带入统一调度页，避免形成三套独立调度逻辑。
- [修复] 个股分析和选股进入定时任务页时保存浏览器任务草稿，从调度页返回后恢复市场、股票或筛选条件及能力选择，避免往返编辑时重复填写。
- [新功能] Web 在主 Agent 与个股分析之间新增独立“市场情报”一级页面，读取最近一次真实大盘复盘与系统数据源目录，可视化展示指数涨跌、市场宽度、板块排行和 Agent 摘要，并明确区分最新/历史快照、已配置/待配置来源；主 Agent 页面继续保持纯对话工作区。
- [改进] 市场情报支持浏览器级自定义看板：用户可开关 Agent 摘要、主要指数、市场宽度和板块排行，并将默认行情、新闻、基本面来源替换或补充为工作区数据源；自定义来源仅展示真实目录详情和连接状态，不虚构尚未接入的指标数据。
- [修复] 专家配置页与共享 Agent 能力面板停止读取旧 `AgentTemplate` / `AgentWorkflowVersion` 目录，移除“工作区自定义能力”历史区块及其旧专家、旧协作工作流；当前业务页面只展示平台预置 Persona、浏览器工作区自定义 Persona 和新的专家团评审预设。
- [改进] Web 侧边栏将原“能力中心”聚合入口展开为 Skill、MCP 工具、数据源和专家配置四个独立页面；各页明确区分平台预置与工作区自定义内容，Skill 新增浏览器工作区自定义草稿，MCP 新增金融工具模板并继续要求管理员填写真实连接地址。
- [改进] 专家群聊不再锁定预设成员；专家团预设只作为快速选择，用户可逐个增删内置或自定义专家、全选或清空，房间参与者与后续运行以最终勾选名单为准，并继续要求至少两位专家。
- [新功能] Web 新增独立“专家评审”工作台，支持单专家咨询与用户指定成员的多专家评审，共享金融 Skill、Tool、MCP 和数据源；后端按公共数据上下文独立运行各 Persona，再由主 Agent 汇总并保存 `ExpertOpinion` 与 `ExpertReview`。
- [改进] Skill 配置明确收敛为完整 Agent 能力中的金融白名单，只向股票研究、选股、组合风险和交易任务发布相关方法；独立 Agent 引擎 式会话、记忆、工具循环、任务恢复与安全边界继续作为底层 Agent 基线，非金融通用 Skill 不进入默认网站目录。
- [改进] 交易页由 Agent 对话改为结构化交易策略与模拟运行台，可配置市场、候选范围、自然语言交易逻辑、信号频率、评估窗口、模拟资金、仓位与日损限制，并挂载 Skill、Tool、MCP、数据源和专家；任务与运行记录保存到后端，当前停留在不生成虚构成交或收益的提案层。
- [新功能] 能力中心新增巴菲特、芒格、段永平、凯西·伍德和张磊五种内置投资 Persona Prompt，以及长期价值、创新与价值辩论、全视角个股委员会三套专家团评审预设；支持在后端工作区版本化 Prompt、自定义专家和专家团，并挂载到研究、选股和专家评审任务。
- [改进] Web 一级业务体验收敛为主 Agent、个股分析、选股和交易四个共享 Agent 底座的工作台；个股分析与选股改为市场/标的或条件优先的结构化任务页，移除策略实验室与 StrategyVersion 验证/运行入口，能力中心继续管理 Skill、MCP、数据源、专家与专家团。
- [改进] 主 Agent 首页按投资决策工作台重新收敛：对话成为唯一主画布，历史会话改为按需抽屉，桌面右侧统一展示当前任务、会话能力和待生成成果；Skill、Tool、健康 HTTP MCP、数据源与专家选择均提交后端校验。导航同步分为投资决策、运行与能力、治理与记录。
- [新功能] 问股首页新增可选独立 Agent 引擎：通过独立 Agent 服务的 OpenAI-compatible API 委托 ReAct、Skill、Tool、MCP、会话与记忆，网站保留股票范围、可见会话和交易安全边界；设置页新增地址、可选 API Key 与只读健康检查，默认 Agent 路径不变。
- [改进] 独立 Agent 引擎 启用后，`/overview` 继续只展示统一的“主 Agent”产品界面，不再在标题、空状态、消息徽标或会话能力面板重复暴露 Runtime 名称；具体后端、连接状态和能力边界集中保留在 Agent 设置页。
- [修复] 主 Agent 首页为 独立 Agent 引擎 使用独立的运行时能力提示与 SSE 适配，不再等待网站内部固定工具；运行中展示阶段和真实耗时，停止操作会中断底层连接并以已取消终态结束，默认部署建议将运行时限设置为 300 秒。
- [改进] Web 前端 MVP 将 `/overview` 重构为主 Agent 对话首页，并新增 Skill、MCP、数据源、专家四个能力中心配置入口：Skill 启用范围会真实约束 Agent 可选和提交的后端 Skill，数据源与专家复用现有 API，MCP 仅保存不含密钥的前端连接草稿并明确标记为预览；新增 `/strategies/new` 四步策略创建向导，当前除现有 Skill 外的会话能力挂载仍不写入后端。
- [新功能] 数据中心新增免密钥 `news:finance_rss` 财经新闻路由，在未配置搜索密钥时作为默认来源，定向读取 Reuters、CNBC、MarketWatch、Yahoo Finance、Financial Times、企业公告线、SEC、Federal Reserve 与金十数据的公开 RSS 标题/摘要/时间/来源/链接，并在空结果或失败时继续使用后续 provider；网页会按财经媒体、企业公告线、监管与宏观分类展示 13 个真实覆盖来源；资讯池同步增加可测试、可启停的 RSS 模板，默认不主动批量抓取。
- [修复] 全站中英文开关统一覆盖当前策略平台：策略首页、策略中心、版本工作台、验证中心、运行中心、数据中心、策略包上传和选股扫描主要工作区会随同一持久化语言状态即时切换，包括操作、状态、空状态、表单及无障碍标签；用户保存的策略名、股票名和后端记录仍保留原始语言。
- [文档] 根 README 改为默认英文，完整简体中文首页迁移至 `docs/README_ZH.md`；同步中英文流程图、语言切换入口和文档索引，保持策略内核、版本验证、运行监控与真实能力边界一致。
- [文档] 重写项目首页 README：从 Daily Stock 的股票分析与通知说明迁移为 LLM TradeBot 的策略内核、运行配置、版本验证和持续研究运行定位，并补充真实产品链路、平台架构、三类输出契约、快速启动与能力边界。
- [新功能] 验证中心接入真实策略版本对比：从同一策略已完成、区间覆盖完整且冻结快照校验通过的正式回放中选择两个不同版本；后端校验历史区间、资金、成本、成交、调仓、市场、引擎和实际回放范围一致后，返回真实收益、回撤、波动、夏普、胜率、换手与成交次数差异。诊断实验、旧版未校验记录、跨策略和口径不一致结果会被明确拒绝，股票池或冻结快照不同时也会提示不能做逐标的归因。
- [新功能] 单股研究、选股扫描、验证中心和运行中心统一展示真实运行记录：每条记录绑定正式策略版本，并显示提交、开始与完成时间；持续运行控制同时显示所用完整策略、最近批次和调度时间。研究和选股在运行中读取任务队列，完成后写回现有分析/筛选历史；回测与运行继续复用验证实验和策略运行批次，不生成第二套运行模型、示例历史或虚假结果。
- [改进] 运行中心重构为完整策略黑盒运行台：围绕“冻结运行配置 → 准备策略输入 → 调用策略内核 → 校验并保存输出”展示运行边界，不再展示多 Agent 拓扑、节点进度或旧全局 Agent 开关；兼容执行器的内部轨迹不再进入批次接口，正式运行完成后只持久化策略声明的标准输出契约。纯规则内核可直接运行，需要模型或数据的内核由各自依赖声明校验。
- [修复] “设置”主页面收敛为当前策略平台真实生效的模型运行时、认证和部署参数；数据源统一跳转数据中心，旧股票池、Daily Agent、旧回测、通知试跑与每日调度继续保留兼容后端但不再冒充当前平台设置。“用量与审计”同步更名为“模型用量”，明确历史 Token 拆分缺口，并为新策略研究运行保存 StrategyVersion 与运行编号归属，旧记录保持未归属而不反向猜测。
- [清理] 移除从 Daily Stock 迁入但当前没有产品用途的“执行与风险”占位页、一级导航入口以及 `/execution`、`/risk` 路由；组合风险分析等现有独立能力不受影响。
- [修复] 默认“研究决策 · A股日线配置”从本地真实日线库冻结具备最低历史覆盖的固定股票池，并采用 OHLCV 可复现的低波动质量规则，现可直接发起正式观察性回放；既有系统配置会生成新的不可变版本安全升级，动态选股版本仍坚持历史时点股票池保护。策略中心同步展示真实“回测就绪 / 需准备历史股票池”状态，不再对所有交易策略统一承诺可直接回测。
- [改进] 策略中心为策略内核和完整策略增加醒目的用途标识，统一区分“单股研究 / 选股扫描 / 交易决策”，并直接说明每类策略的输入、交付结果与交易边界；类型筛选、使用入口和输出契约采用一致术语。
- [修复] 策略工作台与编辑器根据冻结股票池区分“运行回测”和“打开验证中心”，动态选股版本不再承诺当前无法成立的正式历史回放；运行中心和版本工作台改为容忍单个状态接口失败，保留已成功读取的策略、版本和运行记录，并在状态恢复前安全禁用新运行。
- [测试] Playwright 普通 smoke 与策略隔离验收拆分为独立命令；策略验收改用当前“策略内核 → 创建运行配置 → 检查 → 正式发布”链路，并继续使用随机端口、临时密码和隔离 SQLite 数据库。
- [修复] 验证中心不再允许缺少历史时点股票池的动态选股版本先提交、再由后端报错；正式回放会在创建实验前明确禁用，并提供指定股票诊断及创建固定股票池配置入口。后端仍坚持拒绝用当前股票名单冒充历史成分。
- [修复] 按三个 Python 策略内核的真实上游失败与降级行为校正数据依赖：历史日线增强不再被误标为多因子选股必需项，移除 A 股链路未实际消费的情绪数据，并在策略中心区分市场快照、历史日线与候选市场数据标签；带系统审计凭证的早期默认运行配置会安全迁移到当前内核，同名用户策略不被接管。

- [新功能] 自定义 Python 策略内核开放真实受限调用：ZIP 上传静态检查唯一 `run(context)`、输入输出 Schema、数据依赖和标准库白名单；执行使用清洁环境的 `python -I` 独立子进程并施加超时、CPU、文件、进程和输出限制，必需数据缺失结构化失败。三个初始内核迁移为可信 Python 适配器，单股研究与选股页面按正式 StrategyVersion 通过统一入口调用成熟链路。
- [新功能] 三个初始内核分别生成一套普通的 A 股正式运行配置，使单股研究、选股扫描与研究决策开箱即有可选择的完整 StrategyVersion；配置与其他用户策略统一展示、可复制和删除，发布不冒充已经回测验证。
- [修复] 产品策略选择器排除没有可调用策略包的旧架构版本，研究与选股 API 在任务入队前校验正式完整版本与 `executionStatus: ready`，避免选中后才显示无法读取或执行失败。
- [改进] 策略中心重新分离“策略内核 / 完整策略”：三个现有实现与上传包作为可复用只读内核，从内核创建独立 StrategyVersion 运行配置后才可检查、回测和发布；配置页重新展示市场、股票范围、数据源、周期、频率、参数与风险边界，后端阻止内核直接进入回测或运行。
- [新功能] 策略中心开放 ZIP 策略包接入：服务端真实保存并检查目录安全、Manifest、Schema、输出契约和数据依赖，通过后保存为可复用策略内核；用户从内核创建独立 StrategyVersion 草稿并配置市场、股票范围、数据源、周期、运行频率、参数和风险边界，通过当前受限执行器检查后可以正式发布和研究运行。
- [改进] 策略生成指南动态读取当前数据中心目录，复制内容标明可用与未配置来源，并要求 `strategy.yaml` 与 `STRATEGY.md` 使用真实 `sourceId` 声明一致的数据依赖。
- [改进] Web 一级信息架构收敛为“策略运营 / 数据与应用 / 治理与系统”：策略首页改为读取真实策略、验证、运行和数据源 API 的运营摘要，核心链明确为“策略中心 → 验证中心 → 运行中心”；数据中心作为平台依赖入口展示真实连接状态。
- [改进] 用户可见“回测中心”统一改称“验证中心”，继续兼容 `/backtests` 路由并明确当前仅接入交易决策策略的 OHLCV 历史回放；运行中心清理内部 LLM 错误码泄漏，研究报告与选股结果验证仍保持未接入边界。
- [改进] 策略生成指南升级为可直接复制给 Codex 或 Claude Code 的完整交付规范，要求生成策略实现、输入输出 Schema、可重复测试与独立 `STRATEGY.md`；策略中心和版本工作台新增读取真实 StrategyVersion `objective` 的“策略说明”，三个现有正式策略均可直接查看。
- [改进] 产品主链改为完整策略黑盒：主导航移除能力中心，策略配置只公开输入、输出、参数与运行边界，旧 Agent/工作流 API 继续作为历史版本和执行兼容层保留。
- [改进] 策略中心取消“Daily 默认策略 / 我的策略”分组，三个参考 Daily Stock 成熟链路生成的初始正式策略改用普通策略名称，并与其他策略统一展示、筛选、复制和运行；来源只作为实现参考，不再表达所有权分类。
- [清理] 软删除旧网页编排架构生成的趋势突破、质量价值、超跌修复、回踩质量和题材催化策略；没有正在运行的任务被终止，正式版本、验证和审计历史继续保留以便追溯。
- [新功能] 正式策略版本可复制为独立策略草稿，保留冻结配置与内部实现快照，同时生成独立策略身份、节点 ID 和版本历史。
- [新功能] 新增策略生成指南和 `docs/strategy-package-spec.md`，定义 `ResearchReport`、`CandidateList`、`DecisionProposal` 三类契约与代码 Agent 生成提示词；上传代码按标准库白名单和受限子进程边界执行。
- [修复] 策略图保存显式刷新新连接后再生成快照和执行检查，修复关闭 SQLAlchemy autoflush 时最后创建的多节点系统策略被误判为节点孤立。

- [修复] 已发布策略存在后续草稿时仍可在单股研究或选股扫描中选择其冻结正式版本，并修正研究工具页标题与重复持仓行渲染键。
- [修复] 持续研究运行控制器在数据库连接被重建后自动重新绑定当前会话工厂，避免热重载或隔离测试切库时沿用已释放连接导致应用启动失败。
- [测试] 固定策略隔离 E2E 的单次运行标识，使 Playwright 主进程与 worker 使用同一组端口和临时数据库。
- [测试] 更新策略隔离 E2E 以覆盖当前“策略命名弹窗 → 工作流绑定 → 检查 → 正式发布”链路，并串行访问隔离 SQLite 数据库。
- [测试] 同步行情 `preferred_fetcher` 与新闻 `provider` 缓存维度的回归断言，使测试覆盖当前真实数据源路由契约。

- [新功能] 策略中心幂等提供 `Daily · 单股研究策略` 与 `Daily · 选股策略` 两个不可变正式 V1：分别绑定数据库化的 `ResearchReport` / `CandidateList` 工作流，冻结运行时单股范围、A 股 `dual_low` 选股参数、默认真实数据来源及 Daily 兼容执行配置。单股工作流明确拆分确定性证据准备与 LLM 报告，选股工作流明确硬筛、因子评分、可选 LLM 重排及失败回退；初始化不生成报告、候选、收益或交易数据。
- [改进] “单股研究”和“选股扫描”改为策略中心正式版本的产品入口：仅列出匹配 `ResearchReport` / `CandidateList` 契约的已发布 StrategyVersion，在 URL 保留并校验策略与版本身份，读取版本冻结的市场、数据和筛选配置，并统一报告标题层级；前端隔离旧历史报告与其他版本的选股任务，避免把它们误标成当前版本产出。当前仍复用成熟兼容执行器，逐节点工作流执行保持明确待接入。
- [新功能] StrategyVersion 新增研究报告、选股结果和交易决策三类真实产出目标；策略编辑器按目标配置研究范围、筛选已发布工作流并展示冻结输出契约。
- [改进] 已发布工作流从真实终端节点推导 `ResearchReport`、`CandidateList` 或 `DecisionProposal`；草稿保存会冻结所选工作流快照并返回静态检查摘要，策略检查与正式发布拒绝不兼容组合；非交易策略不会进入当前回测或交易研究运行入口。
- [修复] 新策略草稿必须引用能力中心已发布工作流；服务端在每次保存时重新冻结不可变工作流快照，官方策略模板默认绑定标准交易研究工作流，避免引用版本与内嵌节点发生漂移。
- [测试] 新增策略产出切换、工作流契约筛选、研究报告范围、后端契约持久化与非交易策略运行隔离测试。
- [改进] 能力库直接载入并原位新增、编辑、版本化和归档真实 LLM 能力，系统选股、分析、决策与复盘模板不再缩减为两个静态占位项。
- [改进] LLM 能力新增与编辑强制定义有效的 Input/Output JSON Schema；工作流和策略静态检查会拒绝缺少对象类型或字段定义的能力契约。
- [改进] 能力中心收敛为“能力库 / 工作流”两个一级入口，流程编排并入工作流编辑状态；默认展示单股票分析、选股和交易决策三类尚未持久化的能力级工作流蓝图。策略编辑页改为按真实 StrategyVersion 展示市场与标的、数据绑定、一个已发布的主要工作流和策略约束，不再混入未持久化的假想工具节点。
- [改进] 能力库独立展示输入、确定性工具、评级、规则、LLM 与输出契约，并展示策略、回测和运行未来按冻结工作流版本进行黑盒调用的目标关系；现有保存、检查和发布仍复用兼容 Agent 工作流 API，新原子能力持久化、多工作流串联与跨中心统一后端契约仍明确标记为待接入。
- [改进] 新增“乐高式策略”第一版前端预览：Agent 中心升级为能力中心并加入数据、工具、规则、LLM、风险与复合模块目录；策略详情展示读取真实 StrategyVersion 的可组合画布；回测中心展示不含虚假绩效的规则/LLM/混合策略变体实验台。组件持久化、拖拽端口和组合实验 API 均明确标记为后端待接入。
- [改进] Agent 中心收拢能力层级：“复合 Agent”并入 Agent 库，完整 Agent 明确定义为“LLM 配置 + 前置工具图 + 输入输出契约”；现有数据库 Prompt 模板改称 LLM 配置并保留真实版本读写，Agent 层不再重复覆盖 Prompt，并补充契约摘要与旧工作流兼容说明；完整 Agent 持久化仍明确标记为后端待接入。
- [改进] Agent 库新增“完整 Agent 配置”前端预览：以单股研究和选股扫描为例展示默认可选的前置工具图、硬依赖联动、失败策略、EvidencePack、输入输出契约与 LLM 版本继承边界；本阶段不保存工具图，也不触发新的研究或选股后端。
- [改进] 新增独立“研究工具”导航与单股研究入口，复用真实个股分析、历史报告和运行流能力，并明确它不属于策略回测或交易信号；运行中心新增真实批次驱动的运行拓扑、候选推进矩阵和候选研究抽屉，未接入的逐条筛选证据保持明确空状态。
- [改进] Web 设计语言升级为机构级策略研究控制台：全站采用石墨/冷白双主题、钴蓝操作色、连续工作画布、紧凑侧边导航和克制的表面层级，移除旧有霓虹渐变、发光边框、玻璃拟态与过度圆角；策略总览、生命周期导航、按钮、输入框和通用卡片同步统一，并保留桌面与移动端适配。
- [改进] 数据中心登记来源时必须标注数据类型与适用市场（A 股、港股、美股，可多选）；策略草稿先选择市场，再只展示兼容的 K 线、新闻、基本面和其他来源。切换市场会回退不兼容的固定提供方、移除不兼容扩展来源并清空旧市场固定股票，服务端“检查策略”继续阻止跨市场数据源。
- [改进] Web 一级信息架构按“策略工作台 / 能力资产 / 验证与运行 / 治理与系统”重新分组；五个核心页面统一展示 Agent 中心与数据中心支撑策略中心、策略版本进入回测中心并由运行中心执行研究的工作链，主回测入口调整为 `/backtests`，旧 `/validation` 与 `/research` 路由继续兼容。
- [新功能] Agent 中心接入数据库后端：Agent 模板支持创建、不可变版本升级与归档；多 Agent 工作流支持草稿、节点/连接保存、静态检查、正式发布、基于正式版本创建草稿及从旧 StrategyVersion 导入。策略草稿只能选择已发布工作流，并同时冻结工作流版本 ID 与完整 Agent 快照。
- [改进] 策略数据来源支持按类型在“系统自动路由 / 本地缓存 / 已配置提供方”之间切换；目录展示 K 线、新闻、基本面提供方的配置状态和适用市场，选择随 StrategyVersion 冻结，指定来源运行不再跨同类提供方静默降级。
- [改进] 策略数据输入从 Agent 图中拆分为 StrategyVersion 数据来源配置：新模板只包含 ANALYSIS、DECISION、REFLECTION；K 线、新闻、基本面作为开箱即用的默认来源，“其他数据源”从数据与证据目录动态选择；旧正式版本继续兼容，从旧版本创建的新草稿会自动迁移。
- [新功能] 数据与证据页接入持久化数据源目录，可登记无密钥连接标识并供策略草稿的“其他数据源”下拉框选择；系统设置继续独立管理密钥和实际提供方可用性。
- [修复] 正式策略回放与指定股票诊断完成治理隔离：正式回放只能忠实使用 StrategyVersion 股票池；临时代码诊断独立留存，但不会更新验证状态，也不能作为发布关联记录。
- [修复] 固定股票池在“检查策略”时核对 K 线连接和真实输入，缺失代码会阻止发布；研究运行在任何策略 Agent 前完成股票过滤，并跳过范围外数据与全市场选股。
- [修复] 策略级历史回放股票池改为默认跟随 StrategyVersion：支持版本内固定股票池，显式股票代码只作为用户主动选择的实验覆盖；全市场自动选股缺少历史时点股票池时明确停止，避免用当前列表或稀疏缓存制造生存者偏差。
- [修复] 策略级历史回放升级为可信输入契约：完整覆盖请求区间，缺失行情通过现有真实数据源补齐后冻结；执行前校验快照哈希，落实版本仓位上限与 A 股历史费用规则，并将“观察性回放完成”与“验证通过”严格分开。
- [改进] 策略中心列表移除行尾“删除”按钮；草稿和正式版本统一进入具体策略后再删除，并保留已发布、运行中策略的影响确认。
- [改进] 策略草稿在 Agent 链路检查通过后直接提供“运行回测（可选）”与“正式发布”；历史回测不再阻断发布，未回测版本仍明确标记为未经策略级验证。
- [修复] 策略模板复制改用可纠错的命名弹窗，自动建议不冲突的副本名称；重名或 API 创建失败时保留输入并显示真实原因，不再统一误报“请稍后重试”。
- [修复] 六个官方策略模板补齐决策有效期风险规则，模板复制出的草稿现在可直接通过策略检查；策略中心同时明确历史回测为可选验证步骤。
- [新功能] 策略中心新增生命周期感知的策略删除：未发布草稿可直接删除，已发布或正在运行的策略必须确认影响；删除会终止持续运行、取消未完成研究，并保留不可变版本、验证和审计历史。
- [新功能] 研究与验证现提供独立的 StrategyVersion 历史验证后端：冻结策略定义与本地 OHLCV 日线，按“收盘信号、下一交易日开盘成交”执行确定性长仓回放，持久化成本、成交、资金曲线、指标、失败原因与行情快照哈希。
- [改进] 策略中心、研究与验证和运行中心通过真实 StrategyVersion 与验证状态串联；可信的已完成实验会绑定当前策略定义，定义发生实质修改后旧结果自动过期。
- [改进] 研究与验证页面已接入真实实验 API，可配置区间、资金、费用、滑点、市场、调仓频率和股票池来源，并仅在实验完成后展示观察性 OHLCV 回放指标与成交；运行中心区分观察性回放完成、验证通过与未回放状态。
- [改进] 内置选股默认开启；运行中心可直接执行已发布策略，用户仍可在设置中主动关闭选股。
- [改进] 官方策略模板的起步图统一为 ANALYSIS → DECISION → REFLECTION；数据通过版本配置在图外准备，复盘通过 POST_RUN_CONTEXT 在决策之后执行，且不改写当次决策提案。
- [改进] 运行中心在没有可用 LLM 渠道时明确阻止新研究批次并引导用户在设置中配置模型，避免将只有选股但无法分析/决策的运行显示为可用。
- [修复] 策略定义 HTTP smoke 改为从官方策略模板创建，并在结束时软归档其自身和本地分叉测试记录；归档策略的历史自动研究批次不会再出现在运行中心。
- [新功能] 运行中心现在持久化并实时展示每个候选的子 Agent 生命周期；选股完成后立即显示候选，Agent 开始、完成或失败时会更新为可读状态，候选详情自动刷新。
- [修复] 服务重启会将无法恢复的一次性自动研究批次明确标记为中断失败，避免运行中心永久显示“运行中”；持续运行控制仍按持久化状态恢复后续周期。
- [改进] 运行中心移除独立“手动单标的研究”入口；运行一次与持续运行均只执行已发布策略，单标的约束应由策略内 Agent 的输入与 System Prompt 配置。
- [新功能] 运行中心为已发布策略增加“运行一次 / 持续运行 / 暂停运行 / 终止运行”控制；持续控制持久化并按间隔创建可追溯的自动选股研究批次，不涉及订单、成交、持仓或收益。
- [新功能] 正式策略版本可冻结自动选股参数，在运行中心提交 K 线候选扫描并为每个候选持久化独立的 Agent 研究运行；该能力不自动下单、不展示收益。
- [新功能] 策略编辑器支持节点连接点拖放、Schema 字段映射选择器、冲突本地策略分叉与双版本 URL 比较。
- [修复] Intelligence Service 测试恢复测试前环境变量，避免完整套件中的全局环境污染。
- [新功能] 策略定义新增数据库草稿、Agent 图持久化、校验、不可变发布、版本复制、差异和审计闭环。
- [改进] 策略中心与编辑器改用真实定义 API，显示保存、校验和只读状态。
- [测试] 新增策略定义服务与图校验覆盖，并提供 Docker HTTP 冒烟脚本。
- [测试] 策略定义验收脚本支持分阶段 JUnit 证据、隔离 Chromium 生命周期与 30 次 fail-open 回归；新增版本比较和 revision 冲突浏览器场景。
- [修复] 保留策略定义 VERSION_CONFLICT 的结构化响应，使编辑器能暂停自动保存并进入本地分叉流程。
- [改进] 策略编辑器新增持久化策略配置面板，可通过真实 Draft 保存补齐发布所需的决策有效期风险规则。
- [修复] 字段映射编辑器保留新增但尚未完成的映射行，用户可实际选择上下游字段后再写入现有对象契约。
- [测试] 策略定义验收增加认证 10 次同服务与 3 次冷启动证据，并将 Docker 阶段扩展为 Compose 健康检查和 HTTP smoke。
- [改进] 官方 Agent 模板的初始版本提供受控输入/输出 Schema，使字段映射选择器在新建隔离策略中可发现并保存真实字段。
- [修复] POST_RUN_CONTEXT 连接到 Reflection Agent 现在计入图连通性，避免合法发布后上下文被误判为孤立必需 Agent。
- [测试] 策略定义前端专项矩阵扩充至 7 个文件、65 条生产逻辑行为断言；本轮两次隔离 Chromium 核心三场景均为 3 passed、retries=0。完整 `--full` 验收仍未执行，不应据此宣告闭环完成。
- [修复] Intelligence fail-open 回归改用实例级已验证请求边界，避免完整套件中的全局 socket/HTTP mock 清理顺序污染；分红 TTM 测试样本固定在确定窗口内。
- [测试] 验收脚本现将缺失 Docker CLI 记录为可审计的 exit 127 与 failed summary，而不是无产物异常；Docker 阶段改用独立 Compose 模型，避免固定 `stock-server` 名称、宿主 8000 端口与开发 volumes 冲突。
- [测试] 策略定义与版本发布闭环已完成本地完整验收：`20260813T160814Z/summary.json` 为 passed，包含 fail-open 30/30、两次 6262 JUnit Python tests 零失败、前端完整测试、认证 10/10 与冷启动 3/3、核心 Chromium E2E 两轮 3/3，以及 Docker build/health/smoke。GitHub Hosted Actions 尚未触发。
- [测试] 修复 Chromium 流程测试连续添加模板时缺少生产画布状态同步的竞态；重新验收后认证为 10/10 与冷启动 3/3，核心三场景连续两轮均为 3 passed、retries=0；完整 Python 两轮均为 6262 JUnit tests、0 failure/0 error。Docker CLI 仍缺失，不能据此宣告完整 `--full` 通过。

- [新功能] 新增多 Agent 模拟交易页面预览与导航入口。
- [改进] 模拟交易页面可只读关联选股、自选股和单股分析，并明确模拟账本与现有持仓、告警、回测的隔离边界。
- [新功能] 首页新增策略有效性入口，模拟交易页新增预置策略库与策略编排预览，复用既有选股和 Agent Skill 资产。
- [新功能] 新增隔离的模拟策略、不可变策略版本和模拟运行记录 API，为后续多 Agent 运行与纸面账本提供可追溯底座。
- [文档] 新增策略实验室二次开发路线图，明确阶段验收、进度和不自动下单的安全边界。
- [改进] 模拟交易页面可将预置组合保存为独立策略版本并记录预览运行，明确提示记录尚未执行 Agent 或创建订单。
- [新功能] 模拟交易新增六套可直接使用的官方策略模板，显示并保存原有选股策略、分析 Skill、编排模式及默认风控来源。
- [文档] 将策略实验室后续研发拆分为策略编辑、Agent 运行、纸面账本、有效性评估和人工跟随五个可验收阶段。
- [文档] 新增 P3 策略编辑与版本治理的开发 Loop Prompt，沉淀下一阶段的范围、约束和验收标准。
- [新功能] 模拟交易支持从官方模板复制个人策略、编辑关键配置与 Agent Prompt，并以不可变版本保存；新增模板目录、策略读取、版本读取与策略元信息更新 API。
- [文档] 新增 P4 多 Agent 模拟运行时的开发 Loop Prompt，要求运行证据、失败语义和实盘隔离。
- [文档] 新增 P5 纸面执行账本与权益曲线的开发 Loop Prompt，明确只消费已完成模拟运行且与真实资产隔离。
- [文档] 新增 P5.1 纸面成交、持仓与确定性执行的开发 Loop Prompt，承接纸面账户模型。
- [文档] 新增 P5.2 纸面执行服务与权益重算的开发 Loop Prompt，明确 fail-closed、幂等和资金精度约束。
- [文档] 新增 P5.3 纸面账户工作台与拒绝回放的开发 Loop Prompt，采用页面优先的最小闭环。
- [文档] 策略实验室后续改为页面优先的纵向切片交付方式，先确认交互与信息架构，再逐步接通后端能力。
- [改进] 重组全站工作台导航与首页策略状态文案，按策略、研究、复盘和系统分组既有能力，并明确策略有效性尚未形成可跟随结论。
- [改进] Web 工作台统一使用 LLM TradeBot 品牌标题与图标，减少二次开发后仍出现旧项目名称的割裂感。
- [改进] 新增策略库、验证与账本、执行与监控三套前端工作台预览，并将主导航收敛为策略、候选研究、验证和人工执行的主路径。
- [改进] 将首页替换为策略总览，并新增精简的策略编辑器与单次运行回放页面；原个股分析首页保留为非主导航的兼容入口。
- [改进] 按策略中心的信息架构新增实时新闻与数据源工作台，并将“策略库”收敛为模板复用与自定义多 Agent 策略配置入口。
- [新功能] 将 Web 工作台重构为策略总览、策略中心、研究与验证、运行中心、数据与证据、执行与风险六个一级入口，并保留旧路由兼容跳转。
- [新功能] 新增策略、策略版本、Agent 模板/实例、连接、运行、证据、决策提案与候选经验的前端领域类型及策略工作台 Adapter。
- [改进] 策略编辑器改为 Agent 模板库、Agent 编排和单 Agent 配置三栏结构；数据源、风险规则和账本不再被伪装为 Agent 节点。
- [文档] 新增策略中心前端架构说明，记录路由、对象边界、诚实空状态与后端接口方向。
