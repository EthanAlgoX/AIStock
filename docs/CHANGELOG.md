...
- [改进] 模拟收益曲线下的策略按当前累计收益率降序排列，随每 30 秒刷新自动更新排名；无有效收益数据的账户置后，重排保留已展开账户。

- [改进] 交易推演首页改为收益曲线与原位展开的策略条目，移除底部重复详情区，区分曲线勾选与展开操作，详情按需加载并同步五语言。

- [改进] 重整交易推演详情为表现、成交持仓、决策、研究和证据五个栏目，明确区分信号周期与估值方式，保留部分回测提示及真实时间轴。

- [修复] 来源实时模拟成交由执行决策账本映射到策略详情，新增全账户成交记录与现金展示，避免最新估值点无成交被误解为从未交易；缺失单笔费用保持未知，并修正实时 JEV 账户被标为历史回放的文案。

- [改进] 交易推演首页优先展示运行中模拟账户的收益曲线、市场筛选与基准对照，策略详情默认折叠；接入来源规则进化记录并明确候选最终检查状态。

- [新功能] 交易推演增加仅管理员可用的私有运行引擎桥接，在现有页面统一查看、复算与控制来源策略；实例与账本仅保存在服务器。

- [新功能] 交易推演支持基于冻结行情的分段参数实验、回撤约束和候选策略另存，保留原模拟账户与历史，支持五种界面语言。
- [修复] 加密货币回测与模拟比较纳入完整规则参数；新增按完整账户高点计算的回撤曲线。

- [新功能] 注册支持邀请码领取平台每日额度或无邀请码使用个人 LLM API；成员设置支持四家官方服务商，个人调用与平台额度分账、密钥独立保存，失败不回退平台密钥，并同步五语言注册提示。

- [改进] 加密货币作为市场融入雷达、研究、选股、持仓和交易推演，移除独立工作台；三种现货规则复用持久化策略、每日运行和回测账本，支持 UTC 周末日线、小数成交与 365 日年化，并同步五语言入口和使用说明。

- [新功能] 接入 Binance Spot 公开行情、五币小时线研究、高量高波动选币和三种固定规则回测；提供模拟持仓／成交、样本指纹、五语言界面及中英文口径文档。
- [改进] 将加密货币现货能力并入市场雷达、个股研究、策略选股、交易推演和持仓管理原有模块，统一资产切换；数据源显示现货可用性，旧入口跳转至市场雷达。
- [改进] 交易推演详情新增基于已存逐日行情的评测样本指纹与资金归因，展示成交、拒单、费用、滑点和平均资金使用率；仅将相同样本与资金成本口径的完整结果标为可直接比较，同步五语言界面及中英文说明。
- [新功能] 交易推演新增固定版本的高量高波动网格规则后端，历史回测与每日模拟共用收盘信号和次日开盘账本，不调用 LLM/JEV 决策 API；补充五语界面提示与中英文方法说明。
- [修复] JEV 历史回放在单次执行的剩余 Token 预算不足时保留已完成日期并正常结束本批次，下次运行接续未完成日期；单日输入超过完整预算仍明确报错。
- [改进] 投研助理保存的研究与选股 Skill 可转入对应正式任务；任务保存时冻结方法正文，单股报告使用该方法，正式结果后的解读只读取已生成证据，并明确选股 Skill 不改变已发布筛选规则。
- [改进] 交易推演补齐美股、港股分页行业股票目录，A 股全行业遍历板块并核验成分股数量；新增全池量价分位排序后由模型复核前 40 只，保留旧分层抽样并披露缺失数量，同步五语言覆盖提示，保留已确认名单冻结约束，并修复美股带连字符股类代码的市场识别。

- [修复] 美股交易推演选股预览补齐近 20 个交易日的成交量、统计日期和波动率口径，避免模型因未收到成交量证据而拒绝“近一个月高量高波动”范围；不完整样本仍不强行选股。

- [新功能] 持仓录入、关注添加和跟踪配置支持选择 LLM/JEV；JEV 直接输出持仓买卖方向或关注涨跌分类与置信度，不生成报告，复用调度、通知及调用审计并同步五语言界面。

- [改进] 交易推演默认展示最近决策与未成交原因，区分历史验证中断和部分结果，明确日线成交显示时点及已确认范围更新边界；新增提示同步五种语言。

- [修复] 交易推演暂停后修改候选数量并重新预览时，仅提交可编辑的股票范围字段，避免旧预览中的 selection/rule 导致接口拒绝；保存使用新预览名单。

- [改进] 补齐全部 12 个市场的一键复盘、市场上下文与交易日筛选；报告固定文案和 AI 不可用时的数据报告支持五种语言，both 保留原五市场范围以兼容既有定时任务。

- [修复] 市场雷达补齐 12 个市场入口、快照接口和资讯筛选，修复分析订阅语言固定为中文，并同步五语言提示。

- [新功能] 扩展台股、日股、韩股交易推演与官方日线适配，新增英、加、澳、印、德、法个股研究及有界候选选股入口；详见 international-markets.md。
- [修复] 海外日线补充按市场路由，TPEx 成交单位与英国快照报价单位统一；新研究任务和推演解释继承界面语言，补齐五语市场文案。

- [改进] 交易推演支持暂停后完整编辑策略配置，保留旧版历史并为新配置创建独立模拟账户；同步多语言提示和中英文说明。

- [chore] AI 开发协作文件改为仅保留本地，CI 改为验证排除规则；补充凭据、日志、备份和数据库旁路文件的 Git/Docker 忽略规则，保留产品 Skill 与配置模板。

- [修复] 交易推演增加策略及验证记录的停止、删除入口和五语言确认提示；停止撤销运行租约与待执行计划，删除隐藏关联记录并保留审计账本，阻止已停止任务继续记账。

- [改进] JEV 交易配置补充五种语言的默认规则、填写示例及留空说明，支持恢复任务默认设置，清空观察周期和调仓比例时恢复 21 天与 5%。

- [修复] 单股报告统一校验多头入场、止损与目标价，拒绝矛盾价位并阻止原始响应回填；盈亏比由程序计算，补充缺失新闻与估值证据要求。
- [修复] 历史分析上下文按指定日期截断行情，避免读取目标日期后的日线。
- [改进] Web 与已发布策略选股默认严格排名，显式候选轮换不再受随机运行 ID 影响；数据覆盖不足时不标为低风险。

- [改进] 交易推演新建配置默认选择高量高波动网格策略及已有网格参数，保留已保存、复制和投研助理传入的策略选择。

- [修复] 修正日文、繁体中文和韩文告警表单中的简体中文校验提示，并统一繁体股票代碼术语；增加五语言逐项回归测试。

- [修复] 补齐韩语翻译，修正五语言任务账本、能力设置、提示、图表单位与会话导出的混合语言显示，并兼容词条两端空格。
- [测试] 增加五语言词条双向覆盖、占位符及页面切换回归检查；同步 README 语言使用说明。

- [文档] 五语言 README 新增 AI 赋能个人股票研究者的项目愿景，保留具体用途说明。

- [文档] 五语言 README 的研究流程与首次使用示意图统一缩小至 560 像素展示宽度，保留点击查看原图。

- [文档] 五语言 README 前置线上体验入口与邮件申请邀请码链接，补充不同邮箱服务及未配置邮件应用时的手动申请方法，区分在线使用和自行部署的上手流程。

- [文档] 同步繁体中文、日文与韩文 README 的场景介绍、安装与首份报告流程，补齐六张本地化示意图，并校准界面名称和专题文档语言说明。

- [文档] 重写中英文 README，以使用场景和首份报告为主线，新增双语研究流程与上手步骤示意图，明确模型配置和后台运行条件。

- [文档] 五语言 README 补充 JEV 官方访问申请说明、TypeSafe 控制台链接及网站专用配置入口。
- [新功能] 交易推演支持按策略自定义 JEV 判断问题、三类判定条件、补充背景与行情观察天数，配置随策略冻结并进入真实 state/questions 请求；同步五语言界面与回归测试。
- [文档] 新增网站功能与执行逻辑中英对照，明确研究、选股、交易推演、持仓跟踪及通知之间的职责。
- [修复] 新建或取消交易策略时隔离股票范围预览，忽略旧表单的迟到响应，重新预览失败后不能沿用旧确认保存。
- [修复] 持仓与关注股跟踪设置移除重复的运行周期和时间控件，保留通知配置。
...
- [修复] 共享 Agent 聊天 Prompt 和体验请求正确识别韩文，韩文界面的新聊天显式请求韩文回复。

- [新功能] 投研助理支持独立策略创建会话、服务器草稿、完整性检查与版本化 Skill 保存，并可转入现有交易推演配置和范围预览。
- [测试] 覆盖策略草稿恢复、截断回答、并发保存、版本失效、工作区隔离及交易配置转入。

- [改进] 持仓与关注简报分区切换，分别保留名称搜索和市场筛选；统一展示研究状态，关注股运行中及时刷新，新增独立空状态与筛选恢复。

- [改进] 交易范围界面按实际自然语言筛选能力说明，区分预览与保存确认；持仓搜索无结果时提供清除筛选，避免误导重新录入持仓。

- [改进] 交易范围预览先按市场与行业取得候选，再由 LLM 判断自然语言条件；保留市值证据，披露分层取样覆盖，拒绝范围外或重复结果并冻结确认名单。

- [新功能] 单股流水线增加 context_only 采集模式，返回多源分析输入，跳过报告、Agent 和通知，供分类决策服务复用。

- [修复] 关注股票每日曲线保留手动结果优先，关注成果移除交易价位及原始操作副本，持仓研究拒绝空持仓上下文。
- [修复] 股票档案支持同一股票重复查询和失败重试，补全英语界面文案。
- [修复] 窄桌面宽度下较长的英文主导航可横向滚动，避免覆盖品牌与辅助导航。
- [测试] 同步选股字段与分页运行账本测试，补充关注报告、曲线和股票档案重试回归检查。
...
- [新功能] 持仓管理可添加未持有的关注股票，复用独立研究、定时跟踪和评分曲线；关注研究不读取账户、成本或仓位，也不输出持仓操作建议。

- [新功能] 持仓研究冻结当次账户、成本和仓位数据后显式传入个股分析，输出统一的增持、持有、复核、减仓或退出建议及评分；持仓页单独汇总已完成研究的建议轨迹和趋势，不将历史结论带入下一次分析。
- [改进] 持仓建议改为直接使用当次研究的 0–100 综合评分分类，持仓简报和轨迹均保留原始评分；不再将“证据不足待复核”作为常规建议类别。
- [改进] 持仓简报始终同时展示股票名称和代码，名称优先来自股票目录并可回退到研究报告。
- [改进] 持仓跟踪设置改为“保存并开启自动跟踪”，并提供明确的暂停入口，避免仅保存周期和研究策略却未创建定时执行。

- [修复] Agent 策略默认使用范围筛选，按选股、策略、运行风控排列配置，明确股票池可选，并解除已确认范围保存对股票名称目录加载的依赖。

- [新功能] 交易策略支持 Agent、冻结 Skill、自定义交易指令和范围预览，按日执行目标仓位并记录范围、回答及 Token；区分 AI 历史回放与规则回测；行业范围支持最多 32 个中英文匹配词，并按市场选择候选数据源。

- [改进] 交易推演先保存独立策略，再选择历史回测、运行一次或持续模拟并补充验证参数；各次验证独立记账并归属同一策略，兼容已有独立验证记录。
...
- [修复] 创建策略时展示服务器返回的具体参数错误并保留提示，不再误报连接失败或被轮询清除；股票池支持顿号、分号分隔、重复代码去重及数量检查。

- [新功能] 交易推演新增三个可运行的日线规则模板、独立历史回测与持续模拟账户，展示八项业绩指标、基准曲线、每日买卖、持仓与逐股观点；复用私有账本和任务记录，支持暂停、重试及同配置对照。

- [修复] 报告 Markdown 不再将块间原始换行渲染为额外空行，统一收紧圆桌、研究报告与聊天的正文、标题和列表间距；保留代码缩进、显式换行和原始文本。

- [修复] 专家圆桌仅保留正常消息输入，移除单独股票代码框；主持人与专家继承任务市场，未指定标的的讨论使用跨市场提示，避免错误自限为 A 股。

- [新功能] Agent 新增受限表达式计算与绘图工具，聊天和报告支持折线、柱状与条件关系图，保留来源、原始输入、公式和数值表，并支持数值图 SVG 下载。
- [改进] 统一长报告正文层次、行距及表格阅读，移除聊天正文多余留白，大盘复盘增加基于已有指数涨跌幅的对比图；旧报告内容继续兼容。

- [修复] 新用户并发读取专家目录时，以独立保存点处理重复初始化，避免首次打开页面偶发 500，保留用户自定义专家配置。

- [改进] 串联股票档案、候选深研、专家讨论与交易来源；新增可筛选分页运行历史、直接归属 Token 展示及来源跳转，交易启动前冻结真实候选范围并明确仅生成提案。
- [修复] serve-only 启动独立价格轮询但保持每日模型调度关闭；大盘复盘及聊天正式工作流执行前登记运行，失败记录可追查。
- [改进] 选股文案区分正式规则与解读关注点，股票目录加载复用缓存并限制等待时间；补充中英文功能串联说明。

- [新功能] 管理员与成员按 UTC 日期和功能统计 Token，区分确认用量与预留；新增管理员问答、页面操作、模型调用与追踪编号查询，沿用原账本避免重复扣费。

- [改进] 邀请额度改为可动态调整的每日上限，管理后台展示逐邀请码今日用量及近 7 天记录，支持滑杆和精确输入，保留累计账本与全站每日上限。
- [改进] 多用户邀请码改为与邮箱解绑的 7 天一次性凭证，管理员可一次生成 1–20 个，领取者注册时自行填写邮箱。
...
- [新功能] 新增中英文访客演示页与独立邀请试用账户，每人累计 20 万 Token；真实调用默认关闭，逐次预扣/结算并限制全站每日额度，支持管理员邀请、停用和隔离报告历史。
- [新功能] 新增单实例管理员邮箱注册、登录和邮箱修改；初始化凭证防止抢先注册，旧密码验证后补全账户，服务器模式强制 HTTPS，密码或邮箱变更使旧会话失效。
- [新功能] 持仓研究支持填写每隔 1–365 天运行，按市场当地时间调度，并在任务中心显示同一计划；重复保存不重置周期。
...
- [新功能] 增加持仓管理与交易记账页面，复用个股研究生成短简报，自动匹配持仓成本，支持策略/Skill/专家自定义与按市场时区每日后台研究；阈值仅用于风险复核，不执行订单。
- [改进] 增加同一单股研究内核的港股、美股正式配置；持仓运行去重、休市复用和清仓暂停沿用工作区任务与定时计划，错误及行情过期状态明确展示。

- [新功能] 专家使用稳定的本地插画头像；自定义专家支持上传、预览、保存及恢复默认，专家选择、圆桌消息和报告统一展示，图片校验与旧数据库增量兼容。

- [改进] 专家圆桌发言与回应改为章节化阅读，风险和反证突出展示；主持总结直接呈现在时间线，宽版窗口复用同一报告视图，保留发言顺序、轮次、计票及原始记录，不重新生成结论。

- [修复] 专家主持汇总兼容结构化字段为空的历史 JSON 输出：合法字段转为结论、共同判断、风险与研究计划；格式损坏内容仅存原始记录并明确提示，不再回退为正文代码块，也不修补或推断投资结论。

- [改进] 选股报告增加筛选阶段数量与候选评分对照，提前展示降级提示；交易报告改为独立标的计划及建议权益仓位对照，保留条件与未执行边界；两类工作区复用专家评审视图，不改变排序、运行或交易权限。

- [改进] 个股研究中的专家成果聚合为独立评审区：主持汇总、专家立场与自评信心对照、可切换意见和观点/依据/反证分区；合并汇总内相同意见副本，保留不同版本、失败说明及原始记录，不改动前部报告可视化。

- [改进] 个股报告恢复双评分仪表、纵向价位地图和量价指标；四类行动条件提前以双栏图标面板展示，保留双色正反信号区、完整原文及历史快照语义，缺失数值不绘图。

- [改进] 个股正式报告改为投研备忘录与正反证据对照，突出原始结论、条件价位和数据边界，已结束任务信息按需展开，保留历史与原始结果追溯并适配中英文和移动端。

- [新功能] 内置专家扩充至 14 位，新增李录、林奇、朱少醒、谢治宇、柯林斯、李国飞、德鲁克、米勒维尼及利弗莫尔；优化全部默认研究 Prompt，精确升级旧默认并保留自定义、禁用状态和历史快照，同步英文目录与回归测试。

- [修复] 补齐个股研究、策略选股、交易推演、专家圆桌与市场雷达的英文界面，覆盖配置、历史、默认方案、内置能力目录、状态与报告栏目；报告界面标签跟随语言切换，原始研究正文与用户数据保持不变，并增加切换、刷新保持及任务载荷不变的回归验证。
- [改进] 项目品牌更名为「投研团 · InvestCrew」，同步网站导航与页面标题、Web 包元信息、桌面启动页、API 品牌及中英文文档；保留旧会话标识、存储与分发标识以兼容历史数据，远端仓库已更名为 EthanAlgoX/InvestCrew，并同步更新克隆、CI 徽章与设置帮助链接。
- [文档] 更新英文、简体和繁体 README，统一六个工作区与独立专家协作介绍，补充报告优先、默认试用、后台运行及模型配置说明，并注明当前英文界面覆盖范围。
- [改进] 顶部导航新增始终可见的中英文切换按钮；投研助理首页的导航、对话管理、空状态和快捷问题随语言偏好同步切换。
- [改进] 投研助理新建对话时会从近期会话标题和当前会话中的用户提问识别已关注股票，以可用的分析 Skill 生成个性化快捷问题；无法识别时保留默认示例。
- [改进] 统一用户可见工作区命名为市场雷达、投研助理、专家圆桌、个股研究、策略选股和交易推演，避免技术术语和实盘交易暗示；路由、任务类型及历史记录兼容不变。
- [改进] 调整桌面主导航为“市场情报 → 主 Agent → 专家讨论 → 个股分析 → 选股 → 交易”，让市场输入位于主 Agent 左侧、任务工作区位于右侧；手机保持紧凑的五项任务导航。
- [改进] 专家协作角色统一命名为“主持人”，同步页面标签、协作说明与新任务提示词；职责及执行协议不变，历史报告正文保留原文。
- [修复] 专家群聊移除消息容器与专家气泡的双重限宽，回复和输入区充分利用工作区宽度，并加宽完整发言/报告阅读面板，保留手机端与表格横向滚动。
- [改进] 专家讨论采用持续群聊布局，常驻输入框、独立署名消息、跨轮历史与报告卡片；显式调整下轮专家/协作模式时重新校验并冻结配置，不改写历史，默认追问继承旧配置。
- [改进] 专家讨论提升为主 Agent 后的主导航入口，中央对话时间线直接展示专家观点、主管分工/质询、独立投票与模式对应总结；主 Agent 保留简洁状态与最终报告，手机市场情报移入工作区菜单。
- [改进] 统一工作台导航与视觉层级：宽屏辅助入口显示名称，主对话采用连续画布，个股/选股/交易使用目录与报告阅读区，修正历史栏吸顶遮挡并加入键盘跳转入口。
- [改进] 主 Agent 将专家多选和协作方式直接放到对话输入框上方，无需换页；协作进度与报告进入当前会话，后台完成后可继续普通追问，支持历史恢复、会话隔离及删除后不复活。
- [改进] 主 Agent 默认直接对话，专家协作可选且入口常驻；协作配置收敛为具体专家与流水线/辩论/投票模式，移除命名小组的配置和选择入口，旧任务保留并展示成员。
- [新功能] 专家协作支持主管分工流水线、最多两轮辩论、三位独立评审投票；主管仅协调总结，服务端校验分工与计票，平票或票数不足明确未选出，部分失败保留报告与缺口。
- [新功能] 主 Agent 增加专家讨论入口，复用专家讨论工作台和后台记录；支持公共证据、独立专家能力子集、最多两轮真实交叉回应、结论优先报告与关联追问，个股/选股/交易报告可直接邀请专家讨论。
- [改进] 专家讨论冻结成员 Prompt、Skill 内容与报告引用，限制参与人数、并发与运行预算，保留失败阶段和历史结论；旧独立评审协议不迁移，后端协议未就绪时禁止新讨论提交。
- [新功能] 个股、选股和模拟交易支持无填写默认试用：按自选股/固定演示标的与代码板块匹配正式策略、Skill、专家和可用工具，展示匹配依据、降级与成本，复用持久化运行且不覆盖手动配置。
- [修复] 市场识别复用数据源股票代码规范化规则，支持中芯国际 688981.SH 等 A 股交易所前后缀，避免正式个股研究误报市场不一致；保留跨市场与无效代码校验。
- [改进] 任务配置改用紧凑可搜索选择列表：研究/筛选策略单选，Skill/专家/数据工具多选，常驻已选摘要、类型说明、内部滚动与数量上限提示，支持键盘操作且不改变任务绑定。
- [改进] 个股与候选深研合并研究流程/Skill 为单一研究策略入口：预设只读展示方法，自定义组合才选择 Skill；选股区分筛选与深研，交易逻辑与 Skill 组成策略，专家直接可选、工具按需折叠，手动和定时任务不携带隐藏的自选 Skill。
- [改进] 单股研究接入成长质量、事件驱动、趋势量价、预期重估正式配置，Skill 传入报告生成；选股可按原排名深研最多 3 只候选，同一 Run 保存完整报告、部分失败与调用阶段，定时任务保留相同配置。
- [新功能] 交易页接入既有决策信号的风险偏好重评与 1/3/5/10 日后验评估，并提供股票范围内只读信号复盘工具；不把研究信号或偏好评估当作账户审批、模拟成交或真实订单。
- [改进] 专家方向意见明确且双方高置信度冲突时，复用现有冲突检测器触发最多一次证据复核，保留原意见和未解决分歧，不默认启动多轮辩论。
- [改进] 选股工作区接入全部 10 套内置 A 股规则的正式配置，复用同一 Agent 工具与筛选内核，保留已有版本及归档选择；报告补充逐股研究依据、因子评分、催化与失效条件、组合集中度和降级说明。
- [修复] 个股与选股页默认绑定同市场的正式报告流程，无可用版本时启动前阻止，不再静默生成自主讨论；报告视图按明确调用引用归并历史原任务与正式报告，保留原记录且以正式报告为主、解读折叠。
- [修复] 交易提案兼容 `proposals/ticker` 字段并恢复历史原始回答中的结构化提案，展示操作计划、风险与数据缺口；不采信模型生成的审批或成交状态。选股能力失败显示明确原因，不再铺开原始 JSON 或误报空候选成功。
- [改进] 工作区区分执行结束与业务成果，正式空候选、受限、部分成果、待核实和交易提案分别展示；保存真实调用阶段，自主说明不再冒充多种正式报告。
- [改进] 同一工作区运行内的正式研究工具复用当前 Run 保存成果，跨任务成果保留父运行引用；报告可携带有界摘录继续问主 Agent，不自动发送或重跑，内部工作区会话不再混入用户聊天历史。
- [改进] 高级能力配置按需打开，市场行情和研究优先、来源同步详情折叠，统一设置入口名称；专家团明确为独立意见与主持汇总，移除未实现的多轮质询选项。
- [修复] 全市场扫描在任务启动前按引擎声明拒绝不支持的市场；服务重启同步结束活动阶段，后续解读失败仍保留已生成的正式成果。
- [改进] 交易页复用报告优先工作区，默认展示历史模拟运行、交易提案、风险检查与执行记录；策略配置按需展开，启动后自动收起，保留后台状态与取消确认。风险检查区分任务配置校验和提案风险评估，不把提案生成显示为成交或允许真实下单。
- [改进] 个股与选股首页改为报告优先：历史列表、搜索与完整报告默认可见，新建分析配置按需展开，提交后回到报告；研究正文复用结构化报告组件，Markdown 与旧 JSON 片段转换为可读结论、证据、风险及真实指标，原始载荷默认折叠。
- [修复] 个股、选股、专家评审和模拟交易统一恢复后台运行，切页或刷新后保留运行状态与成果；提交期间防重复启动，状态查询失败自动重试，停止操作等待后台确认。
- [新功能] 主 Agent 可发现并调用已发布的单股研究、选股策略；任务页与定时任务可绑定正式版本，原始报告与候选独立保存，专家共享同一份研究结果，报告与运行详情复用结构化展示。
- [改进] 单股策略默认使用确定性证据准备与综合报告，可显式启用内部 Agent；主 Agent 工具返回可回看的完整报告链接，运行详情自动刷新进行中的成果。
- [修复] 单股研究内核区分数据配置与实际覆盖，保留分析历史查询引用，避免把配置启用误报为抓取成功。
- [改进] 主 Agent 桌面端将新建与历史对话提升为常驻会话栏，支持快速创建、切换和管理独立会话；移动端继续通过抽屉访问相同能力。
