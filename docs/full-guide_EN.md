...
| `FRED_API_KEY` | Free [FRED API](https://fred.stlouisfed.org/docs/api/api_key.html) key for official Treasury yield, real-yield, credit-spread, inflation, and employment series | Optional |
...
| `HITHINK_FINANCE_API_KEY` | [HiThink Financial API](https://github.com/HiThink-Tech/Financial-API) key for A-share snapshots, forward-adjusted daily K-lines, and major indices; failures continue through the existing fallback chain | Optional |
...
> **GitHub Actions:** The bundled `00-daily-analysis.yml` maps `TUSHARE_TOKEN`, `FRED_API_KEY`, `HITHINK_FINANCE_API_KEY` / `HITHINK_FINANCE_*`, `TICKFLOW_API_KEY` / `TICKFLOW_*`, and the documented `LONGBRIDGE_*` variables into the job environment. Store API keys in **Secrets**; non-sensitive endpoints, priorities, and timeouts can live in **Variables** or **Secrets**. Longbridge OAuth still requires a client id plus `LONGBRIDGE_OAUTH_TOKEN_CACHE_B64` for headless Actions runs, while the legacy `LONGBRIDGE_APP_KEY` / `LONGBRIDGE_APP_SECRET` / `LONGBRIDGE_ACCESS_TOKEN` triplet remains supported.
...
| `FRED_API_KEY` | Official FRED macro API key; configured observations take priority over market-price proxies | - | Optional |
...
| `HITHINK_FINANCE_API_KEY` | HiThink Financial API key; when configured it joins the A-share daily and realtime provider chains | - | Optional |
| `HITHINK_FINANCE_BASE_URL` | REST base URL; override only for a trusted compatible gateway | `https://fuyao.aicubes.cn` | Optional |
| `HITHINK_FINANCE_TIMEOUT_SECONDS` | Per-request REST timeout in seconds | `15` | Optional |
| `HITHINK_FINANCE_PRIORITY` | A-share daily provider priority; lower values are tried earlier | `0` | Optional |
...
| `REALTIME_SOURCE_PRIORITY` | Comma-separated realtime provider priority. When unset, configured HiThink Financial API/Tushare providers are injected automatically; TickFlow still requires explicit `tickflow`. | See .env.example | Optional |
...
| `MARKET_REVIEW_REGION` | Review markets: cn/hk/us/jp/kr/tw/gb/ca/au/in/de/fr; comma subsets supported; both retains cn,hk,us,jp,kr | `cn` |
...
> Note: `/api/v1/analysis/market-review` task status and persisted history include a structured `market_review_payload`; `region` is the canonical market scope actually executed, alongside fields such as `market_scope`, `sections`, `sectors`, `concepts`, `news`, `market_light`, `indices`, and `macro_indicators`. The macro list contains only observations actually returned by a provider; missing series are never filled with estimated values. Web rendering and history detail use the same structure and fall back to raw `markdown_report` only if the structure is unavailable.
