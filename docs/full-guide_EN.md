...
| `AGENT_BACKEND` | Runtime for ask-stock Chat: `auto` (recommended), `litellm`, `nanobot` (separate Agent runtime), or `codex_app_server` (experimental) | `auto` | No |
| `NANOBOT_API_BASE` | HTTP root exposed by `nanobot serve`, for example `http://127.0.0.1:8900` | - | Required with Nanobot |
| `NANOBOT_API_KEY` | Optional Nanobot API Bearer token; required and matched on both sides for public binds | - | No |
...
> `AGENT_BACKEND=nanobot` delegates ask-stock turns to a separate `nanobot serve` process. Models, runtime skills, tools, MCP, and memory remain configured in Nanobot; the website stores only its API address and optional Bearer token while retaining stock scope, visible sessions, and trading safety boundaries. Website MCP drafts and data-source catalog entries are not automatically synchronized to Nanobot. See [Nanobot Agent Runtime integration](nanobot-integration_EN.md).
