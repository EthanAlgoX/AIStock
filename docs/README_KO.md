...
AI Stock은 시세, 뉴스, 지표 계산과 AI 분석을 한 웹사이트에 모은 도구입니다. 관심 종목을 조사하고 근거와 위험을 살펴본 뒤, 서로 다른 투자 관점을 비교하거나 모의 거래로 전략의 움직임을 확인할 수 있습니다. 리서치는 **중국 본토 A주, 홍콩, 미국, 대만, 일본, 한국, 영국, 캐나다, 호주, 인도, 독일, 프랑스 주식**을 지원합니다. 모의 거래는 앞의 6개 시장에서 지원합니다. 데이터 가용성과 후보 종목 범위는 제공자와 설정에 따라 달라집니다. [시장 가이드(영어)](international-markets_EN.md)를 참고하세요.

한국어 지원은 화면과 출력 언어에 관한 것으로, 한국 주식시장 데이터 지원을 의미하지 않습니다.

## 온라인으로 체험하기 (추천)

먼저 운영 중인 AI Stock 웹사이트를 이용해 보세요. **초대 코드로 가입하면 플랫폼 일일 사용량이 제공됩니다. 코드 없이 가입한 후 내 계정에서 개인 LLM API를 설정해 사용할 수도 있습니다. 직접 배포할 필요가 없습니다.**
...
**언어 선택:** 웹사이트 상단의 언어 메뉴에서 화면 언어를 바꿀 수 있습니다. 화면 문구는 선택한 언어로 표시됩니다. 저장된 보고서, 뉴스 원문과 사용자가 작성한 내용은 원래 언어를 유지합니다. 보고서 출력 언어는 별도로 설정합니다.
...
| 암호화폐 현물 살펴보기 | 시장 레이더, 종목 리서치, 선별, 거래 시뮬레이션, 포트폴리오 | USDT 현물 시세, 코인 선별, 영구 저장 일봉 규칙 백테스트, 모의 보유 |
...
기존 기능의 시장 선택에서 **암호화폐**를 선택하면 같은 전략 저장·일일 모의 실행·백테스트 흐름을 사용합니다. 마감된 UTC 일봉을 사용합니다. 거래소에 실거래 주문을 보내지 않습니다. [현물 가이드(영어)](crypto-spot_EN.md)를 참고하세요.
