...
想先体验？推荐直接使用已上线的 AI Stock 网站。**填写邀请码注册可获得平台每日额度；不填邀请码也能注册，在“我的账户”配置个人 LLM API 即可使用。无需自行部署。**
...
| 研究加密货币现货 | 市场雷达、个股研究、策略选股、交易推演和持仓管理 | USDT 现货行情、选币、持久化日线规则回测和模拟持仓 |
...
在原有模块的市场选项中选择“加密货币”，沿用策略保存、每日模拟和回测流程，使用 UTC 已收盘日线；不会向交易所下单。详见[加密货币现货说明](crypto-spot.md)。
