...
AI Stock 把行情、新闻、指标计算和 AI 分析放进同一个网站。你可以问一只股票、查看依据和风险、比较不同投资风格的观点，再用模拟交易观察策略表现。支持 **A 股、港股和美股**，具体覆盖取决于数据源与所选策略。
...
![从提出股票问题，到收集数据、AI 分析和保存结果的工作流程](assets/readme/how-it-works-zh.svg)
...
**已经有可访问的网站？** 直接打开，跳到[生成第一份报告](#生成第一份报告)。需要自己部署时，再按下面操作。
...
![首次使用四步：连接模型、检查能力、输入股票、阅读报告](assets/readme/first-report-zh.svg)
