...
New trading strategies preselect the high-volume high-volatility grid Skill with a 5-trading-day lookback, a 1.3 minimum volume ratio, a 5% minimum price range, and 5 grid levels. Other methods remain selectable. Existing strategies, copied configurations, and Skills supplied by the research assistant retain their selection. Preselection does not save a strategy or start an account: confirm the stock universe, save, and choose continuous simulation for automatic trading-day updates.
...
API: `GET/POST /api/v1/simulation/portfolios`, `GET /{id}`, `POST /{id}/control` with `run/start/pause`. New strategies require a frozen Agent + Strategy Skill configuration; no executable code. Fixed-rule creation and its templates endpoint are retired, while historical records remain view-only. Existing private databases isolate members and the owner. Background jobs inherit workspace scope and recheck member enablement.
