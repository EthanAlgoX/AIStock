...
AI Stock can delegate the Main Agent model-tool loop to a separate process. The website owns sessions, stock scope, financial capability governance, visible history, and results; the engine owns ReAct, Skills, Tools, MCP, memory, safety limits, and recovery.
...
→ signed AI Stock finance MCP execution
```
...
The default API root is `http://127.0.0.1:8900`. Run the engine in a separate virtual environment or container instead of installing its dependencies into the AI Stock process environment.
...
`AGENT_CAPABILITY_GRANT_SECRET` may be empty for single-process development, which uses a process-random key. Multi-process, replicated, or rolling deployments must configure the same high-entropy value on every AI Stock instance.

After saving and restarting AI Stock, the status card under Agent settings checks `/health` and `/v1/models` without running an Agent turn. `/overview` always presents the unified “Main Agent” interface without exposing the underlying runtime name; the first submitted question is the first real Independent Agent Engine execution. The website consumes Independent Agent Engine's SSE response and shows the active stage, elapsed time, and a stop action. Stopping interrupts the underlying connection and terminates the original request as `cancelled`.
...
| Website stock-analysis skills | AI Stock | Handed off as investment methods and task constraints, never presented as Independent Agent Engine tools |
| Workspace Skills, Tools, MCP, data sources, and experts | AI Stock | Persisted and validated by the backend, then frozen into each Run |
| Website financial Tool Surface | AI Stock | Catalog published at `/api/v1/mcp`; execution also requires a valid signed task grant, exact Tool permission, stock scope, and data-source category |
| Stock scope, visible session, result display | AI Stock | Frozen and stored per turn |
| Trading, approval, and hard risk limits | AI Stock | Independent Agent Engine cannot bypass them; this integration only accepts analysis results |
...
- Only `AGENT_ARCH=single` is supported. AI Stock Multi Agent and Deep Research are not routed through Independent Agent Engine.
